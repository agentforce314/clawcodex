...
- `allow` / `deny` / `ask` are arrays of **rule strings**.
...
- `defaultMode` is one of: `default`, `plan`, `acceptEdits`, `bypassPermissions`, `dontAsk`.
- `additionalDirectories` grants access to paths outside the workspace root.
...
- Hook `type` is one of: `command` (run a shell command), `agent` (run a subagent with a prompt), `http` (POST to a URL), `prompt` (inject a prompt). A hook may also carry `if` (a conditional command gate), `once` (run at most once), and `shell` (the shell to use).
- Example (agent hook): `{ "type": "agent", "prompt": "Verify tests pass: $ARGUMENTS" }`.
