    ...
    *auto-memory* (``MEMORY.md`` via ``load_memory_prompt``), **not** CLAWCODEX.md.
    ...
    if coordinator:
        ...
        blocks = build_full_system_prompt_blocks(
            ...
            # received the "# Non-Interactive Mode" section at all and behaved
            # as if a human would reply (observed on terminal-bench: agents
            # narrating "let me save this for future sessions" mid-trial).
            # Interactive callers (TUI bridge, agent-server) leave the flag
            # unset and are unchanged.
