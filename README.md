...
- **2026-05-21:** **Codebase stats** — Total Python files: 890 files; Total Lines of Python Code: **183,768 lines** (up from 177,428 lines on 2026-05-16; ~+6.3k lines in five days, mostly the `/advisor` multi-provider rewrite + status-bar cost calculation; net −4 files from the `src/tool_system/agent_loop.py` consolidation into `src/query/query.py`).
- **2026-05-21:** **`/advisor` mode → token-efficient coding agent (#181 → #193)** — pair a cheap worker with an expensive reviewer so opus-tier judgment costs roughly Haiku-tier dollars. A typical session: `claude-haiku-4-5` ($1/$5 per Mtok) handles every turn, `claude-opus-4-7` ($5/$25) is consulted only when the worker pauses for a second opinion. Net spend on a verified task ≈ **6× cheaper than opus-only** (the worker uses ~80% of tokens at Haiku rates; the advisor adds ~20% of tokens at Opus rates). Key landed features:
  - **Explicit `<provider>:<model>` syntax** (#192) — `/advisor anthropic:claude-opus-4-7`, `/advisor openai:claude-opus-4-7` (via litellm), `/advisor openrouter:anthropic/claude-opus-4.1`. No name-based inference, no silent misroutes.
  - **Cross-provider routing** (#182/#184/#192) — worker on OpenRouter (`deepseek/deepseek-v4-pro`) + advisor on litellm (`claude-opus-4-7`) verified end-to-end.
  - **Reviewer-quality prompt** (#188) — opus returns critique in `**Gaps:** / **Risks:** / **Do next:**` format, doesn't restate the plan, catches real bugs (e.g. `useParams` returning `string | undefined` instead of the post `id`'s `number`).
  - **Live cost + token visibility** (#190/#191/#193) — status bar shows `tokens 5400/320 · advisor 1326/282 · cost $0.021` so the user can see worker vs advisor token spend and the dollar total at a glance.
  - **`/advisor` slash command + dedicated TUI row** (#181) for set/unset/status reporting.
