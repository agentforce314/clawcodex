...
## F-34: CLI/TUI Frontend 解耦架构（三阶段渐进式重构）
...
| 组件 | 路径 | 说明 |
|------|------|------|
| RuntimeContext | `src/runtime/context.py` | 统一 factory，消除 3 处重复构造 |
| Frontend 协议 | `src/runtime/protocol.py` | 前端契约，实现即接入 |
| AgentEngine | `src/runtime/engine.py` | 统一 agent 循环（替代 AgentBridge + repl 内联） |
| 事件类型 | `src/runtime/events.py` | 标准化的 agent 事件 |
| FrontendRegistry | `src/runtime/registry.py` | 插件式注册 + dispatch |
...
#### Phase 1: RuntimeContext（消除重复构造）

| 里程碑 | 内容 | 工作量 |
|--------|------|--------|
| P1-M1 | `src/runtime/context.py`（RuntimeOptions + RuntimeContext.build()） | 2h |
| P1-M2 | `src/runtime/__init__.py` | 5min |
| P1-M3 | `entrypoints/tui.py` → 使用 RuntimeContext.build() | 30min |
| P1-M4 | `repl/core.py` → ClawcodexREPL 接受 RuntimeContext | 30min |
| P1-M5 | `entrypoints/headless.py` → 使用 RuntimeContext.build() | 30min |
| P1-M6 | 验证：三入口点行为不变 | 30min |

**Phase 1 输出**：三入口点各减 30-50 行，核心拼装代码集中到 1 处。

#### Phase 2: AgentEngine（统一 agent 循环）

| 里程碑 | 内容 | 工作量 |
|--------|------|--------|
| P2-M1 | `src/runtime/events.py`（标准事件类型） | 30min |
| P2-M2 | `src/runtime/engine.py`（AgentEngine） | 4h |
| P2-M3 | `tui/agent_bridge.py` → 内部委派给 AgentEngine | 2h |
| P2-M4 | `repl/core.py` → 使用 AgentEngine | 2h |
| P2-M5 | 集成测试：TUI + REPL submit/cancel/event | 1h |

#### Phase 3: Frontend 协议 + 注册表（插件化）

| 里程碑 | 内容 | 工作量 |
|--------|------|--------|
| P3-M1 | `src/runtime/protocol.py`（Frontend 协议） | 30min |
| P3-M2 | `src/runtime/registry.py`（注册表 + dispatch） | 1h |
| P3-M3 | 实现 ReplFrontend / TuiFrontend / HeadlessFrontend | 2h |
| P3-M4 | `cli.py` → 使用 registry.dispatch() | 1h |
...
| 风险 | 缓解 |
|------|------|
| RuntimeContext.build() 耦合具体 provider/registry | 后期可抽象 ProviderFactory / RegistryFactory |
| AgentEngine 与现有 AgentBridge 行为差异 | 保留 AgentBridge 接口，内部委派，逐步替换 |
| 重构破坏已有功能 | 每个 Phase 完成后执行完整集成测试套件 |
