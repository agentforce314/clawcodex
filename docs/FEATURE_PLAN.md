...
| **PR 检视意见自动修复闭环** | 📋 规划中 | 产品化 `extensions/orchestrator` 的 PR follow-up 能力：读取 PR 网页检视意见、inline comments 与 CI 失败日志，自动修改同一 PR 分支并提交更新 |
...
hooks:
  pre_commit: "echo 'pre-commit verification'"
  pre_push: "echo 'pre-push verification'"
  post_sync: "echo 'post-sync cleanup'"
```
