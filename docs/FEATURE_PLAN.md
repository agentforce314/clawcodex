...
- [一、项目概述](#一、项目概述)
  - [1.1 项目定位](#11-项目定位)
  - [1.2 当前架构（三层解耦）](#12-当前架构（三层解耦）)
- [二、已归档功能模块](#二、已归档功能模块)
- [三、规划功能模块](#三、规划功能模块)
  - [3.1 Orchestrator 自主模式（Symphony 集成）](#31-orchestrator-自主模式（symphony-集成）)
    - [3.1.3 LocalTracker 本地 Issue 文档源设计（F-36）](#313-localtracker-本地-issue-文档源设计（f-36）)
    - [3.1.4 PR 检视意见自动修复闭环设计（F-37）](#314-pr-检视意见自动修复闭环设计（f-37）)
      - [背景与范围](#背景与范围)
      - [架构扩展](#架构扩展)
      - [规范化反馈模型](#规范化反馈模型)
      - [Follow-up 运行流程](#follow-up-运行流程)
      - [Prompt 约束](#prompt-约束)
      - [配置建议](#配置建议)
      - [幂等与安全边界](#幂等与安全边界)
      - [实施切片](#实施切片)
    - [3.1.5 Orchestrator 验证与报告闭环设计（F-38）](#315-orchestrator-验证与报告闭环设计（f-38）)
    - [3.1.6 Issue 重跑入口设计（F-39，label + comment 命令双通道）](#316-issue-重跑入口设计（f-39，label--comment-命令双通道）)
    - [3.1.7 ProgressReporter Sink 协议重构设计（F-40）](#317-progressreporter-sink-协议重构设计（f-40）)
  - [设计目标](#设计目标)
  - [架构对比](#架构对比)
  - [关键组件](#关键组件)
    - [1. ProgressSink 协议](#1-progresssink-协议)
  ...
  - [实施阶段](#实施阶段)
  - [依赖与协同](#依赖与协同)
    - [3.1.8 Coordinator 轻量工具集（F-41）](#318-coordinator-轻量工具集（f-41）)
    - [3.1.9 Shared / Sequential Workspace 策略设计（F-42）](#319-shared--sequential-workspace-策略设计（f-42）)
    - [3.1.10 Tool-call 审计旁路设计（F-45）](#3110-tool-call-审计旁路设计（f-45）)
    - [3.1.11 Issue 会话统一存储与实时介入协议（F-49）](#3111-issue-会话统一存储与实时介入协议（f-49）)
      - [问题现状：两条互不兼容的事件路径](#问题现状：两条互不兼容的事件路径)
      - [目标](#目标)
      - [核心设计](#核心设计)
      - [改造点清单](#改造点清单)
      - [实施阶段总结](#实施阶段总结)
      ...
      - [已拟定的设计决定](#已拟定的设计决定)
      - [依赖与协同](#依赖与协同)
    - [3.1.12 AgentRunner 空转检测机制（F-51）](#3112-agentrunner-空转检测机制（f-51）)
    - [3.1.13 AgentRunner / QueryRunner 运行期可观测性与 stuck-run debug（F-54）](#3113-agentrunner--queryrunner-运行期可观测性与-stuck-run-debug（f-54）)
      - [问题现状](#问题现状)
      - [设计目标](#设计目标)
      - [观测点设计](#观测点设计)
      - [Debug NDJSON](#debug-ndjson)
      - [Timeout diagnostic snapshot](#timeout-diagnostic-snapshot)
      - [Registry / CLI 可见字段](#registry--cli-可见字段)
      - [实施阶段](#实施阶段)
      - [验收标准](#验收标准)
      - [风险与约束](#风险与约束)
      - [依赖与协同](#依赖与协同)
    - [3.1.14 人工检视闸门设计（F-44）](#3114-人工检视闸门设计（f-44）)
  - [3.2 Agent 阶段性进度汇报](#32-agent-阶段性进度汇报)
  - [3.3 Team 成员管理（Phase-7）](#33-team-成员管理（phase-7）)
    - [3.3.1 数据模型](#331-数据模型)
    - [3.3.2 核心机制](#332-核心机制)
    - [3.3.3 实现文件](#333-实现文件)
    - [3.3.4 测试覆盖](#334-测试覆盖)
  - [3.4 结构化输出增强（Outlines）](#34-结构化输出增强（outlines）)
    - [3.4.1 适用场景](#341-适用场景)
    - [3.4.2 数据模型](#342-数据模型)
    - [3.4.3 实现文件](#343-实现文件)
  - [3.5 MCP 扩展功能](#35-mcp-扩展功能)
    - [3.5.1 待增强](#351-待增强)
  - [3.6 Agent 记忆作用域隔离（F-13）（已完成）](#36-agent-记忆作用域隔离（f-13）（已完成）)
  - [3.7 /goal 命令（目标管理）](#37-goal-命令（目标管理）)
    - [3.7.1 功能说明](#371-功能说明)
    - [3.7.2 核心机制](#372-核心机制)
    - [3.7.3 实现文件](#373-实现文件)
    - [3.7.4 数据模型](#374-数据模型)
  - [3.8 ExecuteExtraTool 延迟工具系统](#38-executeextratool-延迟工具系统)
    - [3.8.1 功能说明](#381-功能说明)
    - [3.8.2 核心机制](#382-核心机制)
    - [3.8.3 实现文件](#383-实现文件)
  - [3.9 工具/Skill 调用统计（跨会话）](#39-工具skill-调用统计（跨会话）)
    - [3.9.1 背景](#391-背景)
    - [3.9.2 日志格式](#392-日志格式)
    - [3.9.3 日志字段（统一 schema）](#393-日志字段（统一-schema）)
    - [3.9.4 性能特性](#394-性能特性)
    - [3.9.5 架构设计](#395-架构设计)
    - [3.9.6 查询示例](#396-查询示例)
    - [3.9.7 数据清理](#397-数据清理)
    - [3.9.8 实时查询](#398-实时查询)
    - [3.9.9 替代方案：基于 Transcript 的轻量级统计](#399-替代方案：基于-transcript-的轻量级统计)
    - [3.9.10 基于使用频率的工具/Skill 裁剪](#3910-基于使用频率的工具skill-裁剪)
    - [3.9.11 POS to Agent 转化模式](#3911-pos-to-agent-转化模式)
    - [3.9.12 业务 Agent 长期使用（新窗口重连）](#3912-业务-agent-长期使用（新窗口重连）)
  - [3.10 CreateAgentTool 动态工具创建](#310-createagenttool-动态工具创建)
    - [3.10.1 功能说明](#3101-功能说明)
    - [3.10.2 架构设计](#3102-架构设计)
    - [3.10.3 工具规范（AgentToolSpec）](#3103-工具规范（agenttoolspec）)
    - [3.10.4 三种 call_impl 安全限制](#3104-三种-call_impl-安全限制)
    - [3.10.5 CreateAgentTool 输入规范](#3105-createagenttool-输入规范)
    - [3.10.6 安全性约束](#3106-安全性约束)
    - [3.10.7 持久化机制](#3107-持久化机制)
    - [3.10.8 与现有系统集成](#3108-与现有系统集成)
    - [3.10.9 实现文件](#3109-实现文件)
  - [3.11 sessionStorage 容量限制](#311-sessionstorage-容量限制)
    - [3.11.1 功能说明](#3111-功能说明)
    - [3.11.2 问题场景](#3112-问题场景)
    - [3.11.3 实现文件](#3113-实现文件)
  - [3.12 cacheWarning 容量限制（F-12）](#312-cachewarning-容量限制（f-12）)
  - [3.13 Issue 语义澄清流程（自主模式扩展）](#313-issue-语义澄清流程（自主模式扩展）)
  - [3.14 Auto 模式](#314-auto-模式)
    - [3.14.1 功能说明](#3141-功能说明)
    - [3.14.2 工作原理](#3142-工作原理)
    - [3.14.3 与手动模式的区别](#3143-与手动模式的区别)
    - [3.14.4 循环切换逻辑（已实现部分）](#3144-循环切换逻辑（已实现部分）)
    - [3.14.5 待实现组件](#3145-待实现组件)
    - [3.14.6 分类器 prompt 设计](#3146-分类器-prompt-设计)
    - [3.14.7 实施阶段](#3147-实施阶段)
  - [3.15 Agent 间自主观察与消息交互](#315-agent-间自主观察与消息交互)
  - [3.16 Orchestrator CLI 运维操作界面](#316-orchestrator-cli-运维操作界面)
  - [3.17 CLI 模型供应商与模型切换设计（F-43）](#317-cli-模型供应商与模型切换设计（f-43）)
    - [动机](#动机)
    - [实现设计](#实现设计)
      - [1. 全局注册表](#1-全局注册表)
      - [2. `openai-codex` API 发现钩子](#2-openai-codex-api-发现钩子)
      - [3. 自动注册时机](#3-自动注册时机)
      - [4. `resolve()` 信任已保存配置](#4-resolve-信任已保存配置)
      - [5. 回归静态基线](#5-回归静态基线)
    - [文件变更](#文件变更)
    - [验收](#验收)
  - [3.18 permission_mode enum 正交拆分设计（F-46）](#318-permission_mode-enum-正交拆分设计（f-46）)
    - [目标](#目标)
    - [触发背景](#触发背景)
    - [拆分阶段](#拆分阶段)
    - [F-46.0 设计稿（本期）](#f-460-设计稿（本期）)
    - [F-46.1 规划（后续，等 F-46.0 + F-45 在生产跑一阵）](#f-461-规划（后续，等-f-460--f-45-在生产跑一阵）)
    - [关键设计决定](#关键设计决定)
    - [风险与缓解](#风险与缓解)
    - [子特性](#子特性)
  - [3.19 Permission Settings Schema 重构设计（F-47）](#319-permission-settings-schema-重构设计（f-47）)
  - [3.20 F-48: src/ 核心路径二开修改解耦方案](#320-f-48-src-核心路径二开修改解耦方案)
    - [3.20.1 问题现状](#3201-问题现状)
    - [3.20.2 已完成的解耦模式（可复用）](#3202-已完成的解耦模式（可复用）)
    - [3.20.3 解耦方案：按优先级分 Phase](#3203-解耦方案：按优先级分-phase)
      - [Phase 0: 纯新增文件移入 ext（无风险，立即执行）](#phase-0-纯新增文件移入-ext（无风险，立即执行）)
      - [Phase 1: 注册表/Protocol 扩展消除字段注入（低风险）](#phase-1-注册表protocol-扩展消除字段注入（低风险）)
      - [Phase 2: 子类覆盖模式恢复上游构造器签名（中等风险）](#phase-2-子类覆盖模式恢复上游构造器签名（中等风险）)
      - [Phase 3: 入口点恢复上游逻辑（需谨慎，高集成度）](#phase-3-入口点恢复上游逻辑（需谨慎，高集成度）)
    - [3.20.4 解耦前后效果对比](#3204-解耦前后效果对比)
    - [3.20.5 验收标准](#3205-验收标准)
    - [3.20.6 风险与约束](#3206-风险与约束)
    - [3.20.7 已拟定的设计决定](#3207-已拟定的设计决定)
    - [3.20.8 依赖与协同](#3208-依赖与协同)
  - [3.21 POS 转换器源码固化设计（F-50）](#321-pos-转换器源码固化设计（f-50）)
  - [3.22 Python SDK 方法注册为 Tool（F-52）](#322-python-sdk-方法注册为-tool（f-52）)
    - [背景](#背景)
    - [设计目标](#设计目标)
    - [架构](#架构)
    - [数据模型](#数据模型)
    ...
    - [风险与约束](#风险与约束)
    - [依赖与协同](#依赖与协同)
  - [3.23 Tool 自动暴露为 CLI 斜杠命令（F-53）](#323-tool-自动暴露为-cli-斜杠命令（f-53）)
    - [背景](#背景)
    - [设计目标](#设计目标)
    - [架构](#架构)
    - [命令行格式](#命令行格式)
    ...
    - [风险与约束](#风险与约束)
    - [依赖与协同](#依赖与协同)
- [四、Cron 系统执行引擎（F-22 完整迁移版）](#四、cron-系统执行引擎（f-22-完整迁移版）)
  - [4.1 背景与目标](#41-背景与目标)
  - [4.2 参考实现边界](#42-参考实现边界)
  - [4.3 当前 ClawCodex 状态诊断](#43-当前-clawcodex-状态诊断)
    - [4.3.1 fallback 工具层](#431-fallback-工具层)
    - [4.3.2 下游扩展核心模块](#432-下游扩展核心模块)
    - [4.3.3 关键运行路径断点](#433-关键运行路径断点)
  - [4.4 完整还原的目标行为](#44-完整还原的目标行为)
    - [4.4.0 2026-06 最新 CCB 对比缺口复核](#440-2026-06-最新-ccb-对比缺口复核)
  - [4.5 目标架构](#45-目标架构)
  - [4.6 实施阶段](#46-实施阶段)
  ...
  - [4.7 文件格式](#47-文件格式)
    ...
    - [lock 文件](#lock-文件)
  - [4.8 测试计划](#48-测试计划)
  - [4.9 手工验收流程](#49-手工验收流程)
  - [4.10 实施顺序与完成标准](#410-实施顺序与完成标准)
  - [4.11 CCB 对比发现的补充缺口](#411-ccb-对比发现的补充缺口)
    - [4.11.1 Feature Gate 系统——isKilled 运行时 kill 开关（F-22-G1）](#4111-feature-gate-系统iskilled-运行时-kill-开关（f-22-g1）)
    - [4.11.2 远程 Jitter 实时配置（F-22-G2）](#4112-远程-jitter-实时配置（f-22-g2）)
    - [4.11.3 One-shot 反向 Jitter（整点提前）（F-22-G3）](#4113-one-shot-反向-jitter（整点提前）（f-22-g3）)
    - [4.11.4 Permanent 免过期任务机制（F-22-G4）](#4114-permanent-免过期任务机制（f-22-g4）)
    - [4.11.5 锁注册式清理与 PID 存活探测增强（F-22-G5）](#4115-锁注册式清理与-pid-存活探测增强（f-22-g5）)
    - [4.11.6 工具 Prompt 指引文档增强（F-22-G6）](#4116-工具-prompt-指引文档增强（f-22-g6）)
    - [4.11.7 Analytics 遥测事件注入（F-22-G7）](#4117-analytics-遥测事件注入（f-22-g7）)
    - [4.11.8 inFlight 防重复触发机制（F-22-G8）](#4118-inflight-防重复触发机制（f-22-g8）)
    - [4.11.9 ClawCodex 已有但 CCB 缺失的优势特性（F-22-A1 ~ A5）](#4119-clawcodex-已有但-ccb-缺失的优势特性（f-22-a1-a5）)
    - [4.11.10 补充缺口实施优先级矩阵](#41110-补充缺口实施优先级矩阵)
    - [4.11.11 分析缺口与已有 F22-R/G 交叉映射](#41111-分析缺口与已有-f22-rg-交叉映射)
      - [lastFiredAt 跨进程重启风险（Phase C 增强说明）](#lastfiredat-跨进程重启风险（phase-c-增强说明）)
      - [F-22-G9: SDK daemon 模式（dir / lockIdentity 独立运行）](#f-22-g9-sdk-daemon-模式（dir--lockidentity-独立运行）)
      - [F-22-G10: cronToHuman(utc) UTC 模式显示](#f-22-g10-crontohumanutc-utc-模式显示)
- [五、会话恢复（Session Resume）增强（F-55）](#五、会话恢复（session-resume）增强（f-55）)
  - [5.1 问题现状](#51-问题现状)
  - [5.2 CCB 对比发现的补充缺口](#52-ccb-对比发现的补充缺口)
    - [5.2.1 缺口 1：退出时打印 Resume Hint（S-R1）](#521-缺口-1：退出时打印-resume-hint（s-r1）)
    - [5.2.2 缺口 2：Resume 后历史消息渲染不完整（S-R2）](#522-缺口-2：resume-后历史消息渲染不完整（s-r2）)
    - [5.2.3 缺口 3：`--continue` CLI 快捷命令（S-R3）](#523-缺口-3：-continue-cli-快捷命令（s-r3）)
    - [5.2.4 缺口 4：Resume 时元数据与状态恢复不完整（S-R4）](#524-缺口-4：resume-时元数据与状态恢复不完整（s-r4）)
  - [5.3 补充缺口实施优先级矩阵](#53-补充缺口实施优先级矩阵)
- [六、CCB 对标特性补缺规划](#六、ccb-对标特性补缺规划)
