...
| 编号 | 缺口 | F-22 Phase 关联 | 优先级 | 预计工作量 |
|------|------|----------------|--------|-----------|
| G1 | isKilled 运行时 kill 开关 | Phase E (gate) | P0 | 1-2天 |
| G2 | 远程 Jitter 实时配置 | Phase C (jitter) | P0 | 3-5天 |
| G3 | One-shot 反向 Jitter | Phase C (jitter) | P1 | 2-3天 |
| G4 | Permanent 免过期任务 | Phase B (model) | P1 | 1-2天 |
| G5 | 锁注册式清理与 PID 增强 | Phase C (lock) | P1 | 2-3天 |
| G6 | 工具 Prompt 指引增强 | Phase E (skills) | P2 | 0.5天 |
| G7 | Analytics 遥测事件预留 | 项目级 | P2 | 1天 |
| G8 | inFlight 防重复触发 | Phase C (scheduler) | P2 | 1天 |
| A1~A5 | 已有优势特性保持 | 全 Phase | — | 检查点 (0.5天) |
