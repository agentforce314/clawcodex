...
>
> **v2.4 更新**：新增 3.1.6 Issue 重跑入口设计（F-39）。三种 label 表达重做意图：`agent:retry`（重置本地状态、关旧 PR、重跑整个 issue）、`agent:follow-up`（保留 PR、叠 commit、对应 F-37 follow-up）、`agent:blocked`（永久跳过）；comment 命令 `/agent retry` / `/agent follow-up` 由原作者或 maintainer 触发并限频；CLI 兜底 `issue retry --id 1 --mode reset`。Sub-A label 解析+意图分发，Sub-B 重置重跑，Sub-C follow-up 叠 commit，Sub-D comment 命令解析，Sub-E CLI 兜底，Sub-F 限频+角色校验。
>
> **v2.3 更新**：新增 3.1.5 Orchestrator 验证与报告闭环设计（F-38）。Sub-A 在 `HooksConfig` 增 `pre_commit` / `pre_push` / `post_sync` 三点，git_sync 在 commit/push 前后自动跑 verification gate（默认 `pytest -x`，用户可配 `test_command`）；Sub-B 新增 `report_writer` 生成 Markdown/JSON 报告，`IssueRecord` 增 `report_path` 字段，`git_sync._build_pr_body` 改模板插值；Sub-C 抽象 `TrackerAdapter.update_pull_request`，GitCode 客户端实现 `PATCH /repos/{owner}/{repo}/pulls/{id}`，把报告回写到 PR body 并合并为单条汇总评论；Sub-D 修复 `progress_reporter` 死代码，PhaseComplete 接入 ndjson event log。
>
> **v1.8 更新**：新增 F-37 Orchestrator PR 检视意见自动修复产品化规划。目标是在 issue 自动实现并提交 PR 后，持续读取 PR 网页检视意见、inline comments 与 CI 失败日志，驱动 agent 在同一 PR 分支上自动修改、验证、提交和推送。
>
> **v1.7 更新**：F-34 Phase 1-3 全部完成。CLI parser/dispatch 迁入 `clawcodex_ext/cli`；RuntimeContext 工厂 + Frontend 协议/注册表完成；`ClawCodexExtTUI` 8 个扩展钩子就绪。
...
- [二、已归档功能模块](#二、已归档功能模块)
- [三、Orchestrator 系统（规划中）](#三、orchestrator-系统（规划中）)
  - [3.1 Orchestrator 自主模式（Symphony 集成）](#3.1-orchestrator-自主模式（symphony-集成）)
    - [3.1.3 LocalTracker 本地 Issue 文档源设计（F-36）](#3.1.3-localtracker-本地-issue-文档源设计（f-36）)
    - [3.1.4 PR 检视意见自动修复闭环设计（F-37）](#3.1.4-pr-检视意见自动修复闭环设计（f-37）)
    - [3.1.5 Orchestrator 验证与报告闭环设计（F-38）](#3.1.5-orchestrator-验证与报告闭环设计（f-38）)
    - [3.1.6 Issue 重跑入口设计（F-39，label + comment 命令双通道）](#3.1.6-issue-重跑入口设计（f-39，label-comment-命令双通道）)
    - [3.1.7 ProgressReporter Sink 协议重构设计（F-40）](#3.1.7-progressreporter-sink-协议重构设计（f-40）)
  - [设计目标](#设计目标)
  - [架构对比](#架构对比)
  - [关键组件](#关键组件)
    - [1. ProgressSink 协议（extensions/orchestrator/progress_sink.py）](#1.-progresssink-协议（extensionsorchestratorprogress_sink.py）)
    - [2. CompositeProgressSink 扇出](#2.-compositeprogresssink-扇出)
    - [3. ToolContextProgressSink 默认实现](#3.-toolcontextprogresssink-默认实现)
    - [4. ProgressReporter 兼容 shim](#4.-progressreporter-兼容-shim)
  - [改造点清单](#改造点清单)
  - [进度计算决策表](#进度计算决策表)
  - [并发正确性证明](#并发正确性证明)
  - [验收标准](#验收标准)
  ...
  - [实施阶段](#实施阶段)
  - [依赖与协同](#依赖与协同)
    - [3.1.8 Coordinator 轻量工具集（F-41）](#3.1.8-coordinator-轻量工具集（f-41）)
    - [3.1.9 Shared / Sequential Workspace 策略设计（F-42）](#3.1.9-shared-sequential-workspace-策略设计（f-42）)
    - [3.1.10 Tool-call 审计旁路设计（F-45）](#3.1.10-tool-call-审计旁路设计（f-45）)
    - [3.1.11 Issue 会话统一存储与实时介入协议（F-49）](#3.1.11-issue-会话统一存储与实时介入协议（f-49）)
    - [3.1.12 AgentRunner 空转检测机制（F-51）](#3.1.12-agentrunner-空转检测机制（f-51）)
    - [3.1.13 AgentRunner / QueryRunner 运行期可观测性与 stuck-run debug（F-54）](#3.1.13-agentrunner-queryrunner-运行期可观测性与-stuck-run-debug（f-54）)
    - [3.1.14 人工检视闸门设计（F-44）](#3.1.14-人工检视闸门设计（f-44）)
  - [3.2 Orchestrator CLI 运维操作界面（F-68）](#3.2-orchestrator-cli-运维操作界面（f-68）)
- [四、Agent 核心能力（规划中）](#四、agent-核心能力（规划中）)
  - [4.1 Agent 阶段性进度汇报（F-20）](#4.1-agent-阶段性进度汇报（f-20）)
  - [4.2 Team 成员管理（Phase-7）（F-2）](#4.2-team-成员管理（phase-7）（f-2）)
    - [4.2.1 数据模型](#4.2.1-数据模型)
    - [4.2.2 核心机制](#4.2.2-核心机制)
    - [4.2.3 实现文件](#4.2.3-实现文件)
    - [4.2.4 测试覆盖](#4.2.4-测试覆盖)
  - [4.3 结构化输出增强（Outlines）（F-4）](#4.3-结构化输出增强（outlines）（f-4）)
    - [4.3.1 适用场景](#4.3.1-适用场景)
    - [4.3.2 数据模型](#4.3.2-数据模型)
    - [4.3.3 实现文件](#4.3.3-实现文件)
  - [4.4 MCP 扩展功能（F-3）](#4.4-mcp-扩展功能（f-3）)
    - [4.4.1 待增强](#4.4.1-待增强)
  - [4.5 Agent 记忆作用域隔离（F-13）（已完成）](#4.5-agent-记忆作用域隔离（f-13）（已完成）)
  - [4.6 /goal 命令（目标管理）（F-9）](#4.6-goal-命令（目标管理）（f-9）)
    - [4.6.1 功能说明](#4.6.1-功能说明)
    - [4.6.2 核心机制](#4.6.2-核心机制)
    - [4.6.3 实现文件](#4.6.3-实现文件)
    - [4.6.4 数据模型](#4.6.4-数据模型)
  - [4.7 ExecuteExtraTool 延迟工具系统（F-10）](#4.7-executeextratool-延迟工具系统（f-10）)
    - [4.7.1 功能说明](#4.7.1-功能说明)
    - [4.7.2 核心机制](#4.7.2-核心机制)
    - [4.7.3 实现文件](#4.7.3-实现文件)
  - [4.8 工具/Skill 调用统计（跨会话）（F-75）](#4.8-工具skill-调用统计（跨会话）（f-75）)
    - [4.8.1 背景](#4.8.1-背景)
    - [4.8.2 日志格式](#4.8.2-日志格式)
    - [4.8.3 日志字段（统一 schema）](#4.8.3-日志字段（统一-schema）)
    - [4.8.4 性能特性](#4.8.4-性能特性)
    - [4.8.5 架构设计](#4.8.5-架构设计)
    - [4.8.6 查询示例](#4.8.6-查询示例)
    - [4.8.7 数据清理](#4.8.7-数据清理)
    - [4.8.8 实时查询](#4.8.8-实时查询)
    - [4.8.9 替代方案：基于 Transcript 的轻量级统计](#4.8.9-替代方案：基于-transcript-的轻量级统计)
    - [4.8.10 基于使用频率的工具/Skill 裁剪](#4.8.10-基于使用频率的工具skill-裁剪)
    - [4.8.11 POS to Agent 转化模式](#4.8.11-pos-to-agent-转化模式)
    - [4.8.12 业务 Agent 长期使用（新窗口重连）](#4.8.12-业务-agent-长期使用（新窗口重连）)
  - [4.9 CreateAgentTool 动态工具创建（F-18）](#4.9-createagenttool-动态工具创建（f-18）)
    - [4.9.1 功能说明](#4.9.1-功能说明)
    - [4.9.2 架构设计](#4.9.2-架构设计)
    - [4.9.3 工具规范（AgentToolSpec）](#4.9.3-工具规范（agenttoolspec）)
    - [4.9.4 三种 call_impl 安全限制](#4.9.4-三种-call_impl-安全限制)
    - [4.9.5 CreateAgentTool 输入规范](#4.9.5-createagenttool-输入规范)
    - [4.9.6 安全性约束](#4.9.6-安全性约束)
    - [4.9.7 持久化机制](#4.9.7-持久化机制)
    - [4.9.8 与现有系统集成](#4.9.8-与现有系统集成)
    - [4.9.9 实现文件](#4.9.9-实现文件)
  - [4.10 sessionStorage 容量限制（F-11）](#4.10-sessionstorage-容量限制（f-11）)
    - [4.10.1 功能说明](#4.10.1-功能说明)
    - [4.10.2 问题场景](#4.10.2-问题场景)
    - [4.10.3 实现文件](#4.10.3-实现文件)
  - [4.11 cacheWarning 容量限制（F-12）](#4.11-cachewarning-容量限制（f-12）)
  - [4.12 Issue 语义澄清流程（自主模式扩展）（F-78）](#4.12-issue-语义澄清流程（自主模式扩展）（f-78）)
  - [4.13 Auto 模式 (TRANSCRIPT_CLASSIFIER)（F-16）](#4.13-auto-模式-transcript_classifier（f-16）)
    - [4.13.1 功能说明](#4.13.1-功能说明)
    - [4.13.2 工作原理](#4.13.2-工作原理)
    - [4.13.3 与手动模式的区别](#4.13.3-与手动模式的区别)
    - [4.13.4 循环切换逻辑（已实现部分）](#4.13.4-循环切换逻辑（已实现部分）)
    - [4.13.5 待实现组件](#4.13.5-待实现组件)
    - [4.13.6 分类器 prompt 设计](#4.13.6-分类器-prompt-设计)
    - [4.13.7 实施阶段](#4.13.7-实施阶段)
  - [4.14 Agent 间自主观察与消息交互（F-80）](#4.14-agent-间自主观察与消息交互（f-80）)
- [五、CLI 与配置系统（规划中）](#五、cli-与配置系统（规划中）)
  - [5.1 CLI 模型供应商与模型切换设计（F-43）](#5.1-cli-模型供应商与模型切换设计（f-43）)
    - [动机](#动机)
    - [实现设计](#实现设计)
    - [文件变更](#文件变更)
    - [验收](#验收)
  - [5.2 permission_mode enum 正交拆分设计（F-46）](#5.2-permission_mode-enum-正交拆分设计（f-46）)
    - [目标](#目标)
    - [触发背景](#触发背景)
    - [拆分阶段](#拆分阶段)
    - [F-46.0 设计稿（本期）](#f-46.0-设计稿（本期）)
    - [F-46.1 规划（后续，等 F-46.0 + F-45 在生产跑一阵）](#f-46.1-规划（后续，等-f-46.0-f-45-在生产跑一阵）)
    - [关键设计决定](#关键设计决定)
    - [风险与缓解](#风险与缓解)
    - [子特性](#子特性)
  - [5.3 Permission Settings Schema 重构设计（F-47）](#5.3-permission-settings-schema-重构设计（f-47）)
- [六、Architecture & SDK 下沉（规划中）](#六、architecture-sdk-下沉（规划中）)
  - [6.1 F-48: src/ 核心路径二开修改解耦方案](#6.1-f-48-src-核心路径二开修改解耦方案)
    - [6.1.1 问题现状](#6.1.1-问题现状)
    - [6.1.2 已完成的解耦模式（可复用）](#6.1.2-已完成的解耦模式（可复用）)
    - [6.1.3 解耦方案：按优先级分 Phase](#6.1.3-解耦方案：按优先级分-phase)
    - [6.1.4 解耦前后效果对比](#6.1.4-解耦前后效果对比)
    - [6.1.5 验收标准](#6.1.5-验收标准)
    - [6.1.6 风险与约束](#6.1.6-风险与约束)
    - [6.1.7 已拟定的设计决定](#6.1.7-已拟定的设计决定)
    - [6.1.8 依赖与协同](#6.1.8-依赖与协同)
  - [6.2 POS 转换器源码固化设计（F-50）](#6.2-pos-转换器源码固化设计（f-50）)
  - [6.3 Python SDK 方法注册为 Tool（F-52）](#6.3-python-sdk-方法注册为-tool（f-52）)
  - [6.4 Tool 自动暴露为 CLI 斜杠命令（F-53）](#6.4-tool-自动暴露为-cli-斜杠命令（f-53）)
- [七、Cron 系统执行引擎（F-22 完整迁移版）](#七、cron-系统执行引擎（f-22-完整迁移版）)
  - [7.1 背景与目标](#7.1-背景与目标)
  - [7.2 参考实现边界](#7.2-参考实现边界)
  - [7.3 当前 ClawCodex 状态诊断](#7.3-当前-clawcodex-状态诊断)
    - [7.3.1 fallback 工具层](#7.3.1-fallback-工具层)
    - [7.3.2 下游扩展核心模块](#7.3.2-下游扩展核心模块)
    - [7.3.3 关键运行路径断点](#7.3.3-关键运行路径断点)
  - [7.4 完整还原的目标行为](#7.4-完整还原的目标行为)
    - [7.4.0 2026-06 最新 CCB 对比缺口复核](#7.4.0-2026-06-最新-ccb-对比缺口复核)
  - [7.5 目标架构](#7.5-目标架构)
  - [7.6 实施阶段](#7.6-实施阶段)
    - [Phase A — runtime-first 接线 ✅ 已完成](#phase-a-—-runtime-first-接线-✅-已完成)
    - [Phase B — 存储与模型语义对齐](#phase-b-—-存储与模型语义对齐)
    - [Phase C — scheduler 语义对齐](#phase-c-—-scheduler-语义对齐)
    - [Phase D — 执行队列与结果追踪](#phase-d-—-执行队列与结果追踪)
    - [Phase E — skills 与用户命令](#phase-e-—-skills-与用户命令)
    - [Phase F — teammate / agent ownership](#phase-f-—-teammate-agent-ownership)
  - [7.7 文件格式](#7.7-文件格式)
    - [durable task 文件](#durable-task-文件)
    - [lock 文件](#lock-文件)
  - [7.8 测试计划](#7.8-测试计划)
  - [7.9 手工验收流程](#7.9-手工验收流程)
  - [7.10 实施顺序与完成标准](#7.10-实施顺序与完成标准)
  - [7.11 CCB 对比发现的补充缺口](#7.11-ccb-对比发现的补充缺口)
    - [7.11.1 Feature Gate 系统——isKilled 运行时 kill 开关（F-22-G1）](#7.11.1-feature-gate-系统——iskilled-运行时-kill-开关（f-22-g1）)
    - [7.11.2 远程 Jitter 实时配置（F-22-G2）](#7.11.2-远程-jitter-实时配置（f-22-g2）)
    - [7.11.3 One-shot 反向 Jitter（整点提前）（F-22-G3）](#7.11.3-one-shot-反向-jitter（整点提前）（f-22-g3）)
    - [7.11.4 Permanent 免过期任务机制（F-22-G4）](#7.11.4-permanent-免过期任务机制（f-22-g4）)
    - [7.11.5 锁注册式清理与 PID 存活探测增强（F-22-G5）](#7.11.5-锁注册式清理与-pid-存活探测增强（f-22-g5）)
    - [7.11.6 工具 Prompt 指引文档增强（F-22-G6）](#7.11.6-工具-prompt-指引文档增强（f-22-g6）)
    - [7.11.7 Analytics 遥测事件注入（F-22-G7）](#7.11.7-analytics-遥测事件注入（f-22-g7）)
    - [7.11.8 inFlight 防重复触发机制（F-22-G8）](#7.11.8-inflight-防重复触发机制（f-22-g8）)
    - [7.11.9 ClawCodex 已有但 CCB 缺失的优势特性（F-22-A1 ~ A5）](#7.11.9-clawcodex-已有但-ccb-缺失的优势特性（f-22-a1-a5）)
    - [7.11.10 补充缺口实施优先级矩阵](#7.11.10-补充缺口实施优先级矩阵)
    - [7.11.11 分析缺口与已有 F22-R/G 交叉映射](#7.11.11-分析缺口与已有-f22-rg-交叉映射)
- [八、会话恢复（Session Resume）增强（F-55）](#八、会话恢复（session-resume）增强（f-55）)
  - [8.1 问题现状](#8.1-问题现状)
  - [8.2 CCB 对比发现的补充缺口](#8.2-ccb-对比发现的补充缺口)
    - [8.2.1 缺口 1：退出时打印 Resume Hint（S-R1）](#8.2.1-缺口-1：退出时打印-resume-hint（s-r1）)
    - [8.2.2 缺口 2：Resume 后历史消息渲染不完整（S-R2）](#8.2.2-缺口-2：resume-后历史消息渲染不完整（s-r2）)
    - [8.2.3 缺口 3：`--continue` CLI 快捷命令（S-R3）](#8.2.3-缺口-3：-continue-cli-快捷命令（s-r3）)
    - [8.2.4 缺口 4：Resume 时元数据与状态恢复不完整（S-R4）](#8.2.4-缺口-4：resume-时元数据与状态恢复不完整（s-r4）)
  - [8.3 补充缺口实施优先级矩阵](#8.3-补充缺口实施优先级矩阵)
- [九、CCB 对标特性补缺规划](#九、ccb-对标特性补缺规划)
  - [F-60: Pipe IPC + LAN 群控系统](#f-60-pipe-ipc-lan-群控系统)
    - [背景](#背景)
    - [子特性分解](#子特性分解)
    - [架构建议](#架构建议)
    - [依赖](#依赖)
  - [F-61: Computer Use 屏幕操控](#f-61-computer-use-屏幕操控)
    - [背景](#背景)
    - [子特性分解](#子特性分解)
    - [架构建议](#架构建议)
    - [依赖](#依赖)
  - [F-62: Chrome 浏览器自动化控制](#f-62-chrome-浏览器自动化控制)
    - [背景](#背景)
    - [子特性分解](#子特性分解)
    - [架构建议](#架构建议)
  - [F-63: Channels 频道通知系统](#f-63-channels-频道通知系统)
    - [背景](#背景)
    - [子特性分解](#子特性分解)
    - [架构建议](#架构建议)
  - [F-64: Voice Mode 语音输入](#f-64-voice-mode-语音输入)
    - [子特性分解](#子特性分解)
    - [实现建议](#实现建议)
  - [F-65: Langfuse Agent 可观测性](#f-65-langfuse-agent-可观测性)
    - [背景](#背景)
    - [子特性分解](#子特性分解)
    - [架构建议](#架构建议)
  ...
  - [clawcodex 对比 CCB 的领先优势](#clawcodex-对比-ccb-的领先优势)
    ...
    - [优势 3: POS-to-Agent 编译器](#优势-3-pos-to-agent-编译器)
    - [优势 4: LiteLLM Provider（100+ 模型统一接口）](#优势-4-litellm-provider（100-模型统一接口）)
    - [优势 5: Manager/Worker 增强通信（TaskInspect/TaskDirectives）](#优势-5-managerworker-增强通信（taskinspecttaskdirectives）)
## 项目级二开边界约束
...
### 3.2 Orchestrator CLI 运维操作界面（F-68）
...
### 4.7 ExecuteExtraTool 延迟工具系统（F-10）
**状态**: ⏳ 待实现
**目标**: 按需加载延迟工具，支持语义搜索
...
#### 4.8.9 替代方案：基于 Transcript 的轻量级统计
...
### 4.9 CreateAgentTool 动态工具创建（F-18）
**状态**: 🔄 规划中
...
### 4.13 Auto 模式 (TRANSCRIPT_CLASSIFIER)（F-16）
**状态**: ⏳ 待实现
**优先级**: P2
...
### 5.2 permission_mode enum 正交拆分设计（F-46）
**状态**: ⏳ 规划中
**优先级**: P2
...
### 5.3 Permission Settings Schema 重构设计（F-47）
...
---
### 6.3 Python SDK 方法注册为 Tool（F-52）
**状态**: 📋 规划中
**优先级**: P2
...
#### 7.11.1 Feature Gate 系统——isKilled 运行时 kill 开关（F-22-G1）
**优先级**: P0
...
#### 7.11.3 One-shot 反向 Jitter（整点提前）（F-22-G3）
**优先级**: P1
...
#### 7.11.4 Permanent 免过期任务机制（F-22-G4）
**优先级**: P1
...
#### 7.11.5 锁注册式清理与 PID 存活探测增强（F-22-G5）
**优先级**: P1
...
#### 7.11.6 工具 Prompt 指引文档增强（F-22-G6）
**优先级**: P2
...
#### 7.11.7 Analytics 遥测事件注入（F-22-G7）
**优先级**: P2
...
#### 7.11.8 inFlight 防重复触发机制（F-22-G8）
**优先级**: P2
...
#### 7.11.9 ClawCodex 已有但 CCB 缺失的优势特性（F-22-A1 ~ A5）
...
### 8.2 CCB 对比发现的补充缺口
#### 8.2.1 缺口 1：退出时打印 Resume Hint（S-R1）
...
#### 8.2.2 缺口 2：Resume 后历史消息渲染不完整（S-R2）
...
#### 8.2.3 缺口 3：`--continue` CLI 快捷命令（S-R3）
...
#### 8.2.4 缺口 4：Resume 时元数据与状态恢复不完整（S-R4）
