...
> 详细设计与落地记录已归档至 [ARCHIVED_FEATURES.md §二十一.10 F-34 CLI/TUI Frontend 解耦架构](./ARCHIVED_FEATURES.md#二十一10-f-34-clitui-frontend-解耦架构)。
...
> - 已完成的 LocalTracker（F-36）、验证与报告闭环（F-38）、Issue 重跑入口（F-39）、Coordinator 轻量工具集（F-41）、Shared / Sequential Workspace（F-42）与 Tool-call 审计旁路（F-45）详见 [ARCHIVED_FEATURES.md §二十一](./ARCHIVED_FEATURES.md#二十一2026-06-02-已实现功能归档)。
...
**状态**: ✅ 完成
**优先级**: P0
**跟踪文档**: `docs/PROGRESS.md` → `F-51: AgentRunner 空转检测机制`

##### 背景

Orchestrator 处理 issue 时，若该 issue 的 deliverables 已在 base branch 中存在（例如通过上游 commit 预置、或在 shared workspace 中被前一个 issue 实现），agent 会陷入无意义循环：反复运行 `python3 --version` / `date` 等 busy-work 命令 → 无文件变更 → session 持续 countinue → 耗尽 max_turns → retry → 再循环。
...
#### 3.1.13 人工检视闸门设计（F-44）

**状态**: ✅ 完成
**优先级**: P1
**跟踪文档**: `docs/PROGRESS.md` → `F-44: Orchestrator 人工检视闸门（Review Gate）`

##### 背景

Orchestrator 完成自动开发后，代码变更直接进入 PR 状态。对于生产环境，需要人工检视闸门（review gate）确保：
1. 代码变更在合并前经过人工审查
2. 审查者可以查看 diff、运行验证、审批或驳回
3. 驳回时可附加反馈，自动触发 agent retry（F-39 复用）
...
### 3.16 permission_mode enum 正交拆分设计（F-46）
...
### 3.17 Permission Settings Schema 重构设计（F-47）
...
实现策略：
1. `ast.parse()` 逐文件解析，提取类定义和顶层函数
2. `inspect.getdoc()` 等价逻辑提取 docstring（Google/NumPy/reST 兼容）
3. `typing.get_type_hints()` 或 `ast.unparse()` 提取参数类型注解
4. import 语句分析提取模块间依赖关系

##### 子模块二：增强 SkillGrouper 策略（`extensions/pos_converter/skill_grouper.py` 增量）

现有 `SkillGrouper` 新增分组策略枚举：
...
class AgentMarkdownWriter:
    """从 AgentDefinition + SkillSpec[] 生成 CLI 可加载的 .claude/agents/*.md。"""

    def write_agent(self, agent_def: AgentDefinition, output_dir: Path) -> Path
        """生成 <name>.md，包含完整 frontmatter + system prompt。"""
        # frontmatter: name / description / model / tools / skills / when_to_use
        # body: 参考说明 + 使用约束
    ...
    def write_workflow(self, name: str, skills: list[SkillSpec], output_dir: Path) -> Path
        """可选：生成 orchestrator WORKFLOW.md，含 tracker/workspace/agent 配置骨架。"""
```
...
```
总览 Agent（clawcodex-overview）
├── frontmatter name: "clawcodex-overview"
├── 职责: 理解整个 POS 工作流，委派任务给子 Agent
├── system prompt 包含:
│   ├── 工作流概述（阶段、顺序、依赖关系）
│   ├── 子 Agent 清单（名称 + 职责 + 何时调用）
│   ├── 跨组件编排策略（数据流转、异常处理）
│   └── 使用示例（常见任务 → 调用链）
├── tools: ["*"]          ← 可访问所有工具
├── skills: [所有 Skill]   ← 可调用所有技能
└── 可被 `@clawcodex-overview` 调用；启动时自动检测并作为默认 Agent 加载（参见下一节）
```

生成逻辑在 `AgentMarkdownWriter` 中新增：
...
class AgentComponentInfo:
    ...
    capabilities: list[str]         # ["转码", "切片", "水印叠加", "画质检测"]
    input_types: list[str]          # ["mp4", "avi", "raw"]
    output_types: list[str]         # ["hls", "mp4"]
...
class WorkflowStage:
    ...
    depends_on: list[str] | None    # ["数据接入"] → 前置阶段
    output_type: str                # "标准化视频"
```

##### 启动时默认 Agent 替换机制

生成的 `clawcodex-overview.md` 需要能被 clawcodex **加载为默认 Agent**（替换通用 Claw Codex agent），而非仅通过 `@clawcodex-overview` 手动调用。

**设计方案：自动检测 + `--agent` CLI 参数覆盖**
...
总览 Agent 的 frontmatter 中使用 `name: clawcodex-overview`，文件名推荐为 `clawcodex-overview.md`。

```
.claude/agents/
├── clawcodex-overview.md    ← 自动作为默认 Agent（命名约定）
├── video-ops-agent.md       ← @video-ops-agent 可调用
├── data-process-agent.md    ← @data-process-agent 可调用
└── quality-check.md         ← @quality-check 可调用
```
...
```
启动流程:
  1. 加载所有 .claude/agents/*.md → agent_definitions[]
  2. 检查是否有 --agent CLI 参数
     → 有：用 find_agent_by_type(agents, agent_type) 定位
     → 无：扫描是否存在 clawcodex-overview.md → 返回 agent definition
  3. 如果找到 default_agent:
     → 将 default_agent.get_system_prompt() 作为 append_system_prompt
       追加到 build_full_system_prompt() 的末尾，保留所有标准节
  4. 否则：使用 GENERAL_PURPOSE_AGENT（当前行为，行为不变）
```

**改造点清单**（`extensions/pos_converter/default_agent.py`，新增）：

| 文件 | 改动 | 影响 |
|------|------|------|
| **新增**: `extensions/pos_converter/default_agent.py` | `resolve_default_agent(cwd)` 扫描 `.claude/agents/clawcodex-overview.md`；`resolve_agent_by_type(cwd, agent_type)` 供 `--agent` 参数使用 | 新增 |
| `clawcodex_ext/cli/parser.py` | 新增 `--agent <agent-type>` 参数，通过 RuntimeOptions 透传 | 向后兼容 |
| `clawcodex_ext/runtime/context.py` | `RuntimeOptions` 新增 `default_agent: str \| None = None` 字段 | 向后兼容 |
| `clawcodex_ext/frontend/repl.py` | 从 ctx.options 提取 `default_agent` 传给 `ClawCodexExtREPL` | 向后兼容 |
| `clawcodex_ext/repl/app.py` | `ClawCodexExtREPL.__init__` 接受 `default_agent`；`chat()` 中调用 `resolve_default_agent()` 或 `resolve_agent_by_type()`，将 agent body 追加到 style_prompt/append_system_prompt | 向后兼容 |
| `src/repl/core.py` | `chat()` 中非 coordinator 模式下增加 default_agent 注入逻辑 | 向后兼容（上游 REPL 不受影响） |

**启动时 Agent 标识 Banner（2026-07 补充）**：
以上机制落地后，为了提升用户透明度——让用户一眼确认当前使用的是自定义 Agent 而不是通用兜底 Agent——在 `_resolve_startup_agent()`（`clawcodex_ext/cli/dispatch.py`）中增加了启动 banner：

```
⚡ Using agent: ascend-dataforge (28 sub-agents)
```

- 当解析到的 agent 的 `skills` 列表中有 `skill-` 前缀项时，统计为 sub-agent 数量并显示在 banner 中
- 无自定义 Agent（通用兜底）时不输出，保持零 banner 启动
- 所有输出走 `stderr`，不干扰 stdout 管道
- 文件变更仅涉及 `clawcodex_ext/cli/dispatch.py` 一处

此机制确保：
- **纯新目录 / 无 POS 项目** → clawcodex 行为不变（默认通用 agent）
- **已初始化 POS 项目**（有 `clawcodex-overview.md`）→ 自动使用总览 Agent 的 body 作为附加系统指令
- **用户可覆盖** → `--agent general-purpose` 仍可回到默认行为
- **启动可见性** → 用户可立即确认自定义 Agent 已生效及 sub-agent 数量
- **向后兼容** → `resolve_default_agent()` 未找到文件时返回 None，不改变现有启动路径
...
├── .claude/
│   └── agents/
│       ├── clawcodex-overview.md    ← 总览 Agent（自动加载为默认入口）
│       ├── video-ops-agent.md       ← 组件 Agent
│       ├── data-process-agent.md
│       ├── quality-check.md
│       └── ...
└── .atomcode/
    └── skills/
...
总览 Agent 与非总览 Agent 在格式上无区别——都是标准的 `.claude/agents/*.md` 格式，通过 `name` frontmatter 字段标识身份。

##### 现有 `pos_converter` 的增量改造

| 文件 | 改动 | 影响 |
|------|------|------|
| `extensions/pos_converter/__init__.py` | 导出 `SourceCodeParser`, `SourceComponent`, `AgentMarkdownWriter` | 无兼容问题 |
| `extensions/pos_converter/sdk_parser.py` | 保留 `SdkParser` 不动，新增 `SourceCodeParser` 类 | 新增 |
| `extensions/pos_converter/skill_grouper.py` | 新增 `GroupStrategy` enum；`SkillGrouper.__init__` 增加 `strategy` 参数；新增 `_component_group()` / `_io_relation_group()` | 向后兼容，旧接口不变 |
| `extensions/pos_converter/agent_builder.py` | `AgentBuilder.build()` 增加 `format` 参数（`agent_definition` / `markdown` / `both`）；新增 `_write_agent_markdown()` 调用 `AgentMarkdownWriter` | 向后兼容 |
| `extensions/pos_converter/templates.py` | 新增 `.claude/agents/*.md` 和 `.atomcode/skills/*/SKILL.md` 的 Jinja2 模板 | 新增 |
| **新增**: `source_parser.py` | `SourceCodeParser` + 数据类 | 新增 |
| **新增**: `agent_md_writer.py` | `AgentMarkdownWriter` + 模板渲染 | 新增 |
| `extensions/pos_converter/agent_md_writer.py` | 新增 `write_overview_agent()` 方法；新增 `AgentComponentInfo` / `WorkflowStage` 数据类 | 增量 |
| `extensions/pos_converter/agent_builder.py` | `AgentBuilder.build()` 在检测到多组件时自动调用 `write_overview_agent()`，无参数开关 | 向后兼容 |
| **新增**: `extensions/pos_converter/default_agent.py` | `resolve_default_agent(cwd)` 扫描 `.claude/agents/clawcodex-overview.md`；`resolve_agent_by_type(cwd, agent_type)` 供 `--agent` CLI 参数使用 | 新增 |

##### CLI 集成增强（`clawcodex_ext/cli/pos_cmd/commands.py` 增量）
...
clawcodex-dev pos convert ./path/to/src       # 新增：Python 源码目录
                     ...
                     --skills .atomcode/skills # 输出 skill 定义
                     --workflow               # 额外生成 WORKFLOW.md
                     --name my-agent          # 指定 agent 名称（各组件主体名）
                     --strategy component     # 分组策略（keyword/component/io）
clawcodex-dev --agent clawcodex-overview      # 以总览 Agent 为默认 agent 启动
clawcodex-dev --agent general-purpose         # 回到通用 agent（默认行为）

总览 Agent **始终**生成——用户无需额外参数。只要输出目录包含多个组件，`AgentBuilder.build()` 自动调用 `write_overview_agent()` 创建 `clawcodex-overview.md`。
```

##### 验收标准

1. 给定 AscendDataForge 的 `组件/视频算子/` 目录，`clawcodex-dev pos convert ./组件/视频算子 --out .claude` 生成可被 CLI `@video-ops-agent` 加载的 `.claude/agents/video-ops-agent.md`
2. `clawcodex-dev pos convert ./组件`（多组件的上层目录）自动生成 `.claude/agents/clawcodex-overview.md`，包含工作流概述和所有子 Agent 的委派指引
3. `SourceCodeParser` 正确提取类名、方法名、参数、docstring、import 依赖
4. 生成的 Agent markdown 符合 `load_agents_dir.py` 的解析格式（frontmatter `name`/`description`/`tools`/`skills`）
5. 生成的总览 Agent 包含 `AgentComponentInfo` 和 `WorkflowStage` 结构化描述
6. 生成的 `SKILL.md` 可在 CLI 中通过 `@skill-name` 调用
7. 所有新增代码通过 `python3 -m pytest tests/test_pos_converter*.py -q`
8. 现有 `extensions/pos_converter` 测试继续通过
9. `resolve_default_agent()` 检测到 `clawcodex-overview.md` 时返回对应的 agent definition；未找到时返回 None，不改变启动行为

##### 风险与约束

| 风险 | 缓解 |
|------|------|
| Python AST 跨版本差异 | 锁定 `ast` 标准库，不依赖第三方；CI 用 Python 3.9+ |
| 大目录扫描性能 | `SourceCodeParser` 支持 `exclude_patterns`；`max_depth` 控制递归深度 |
| docstring 格式不一 | 统一提取首段作为 description，参数字段尝试 Google/NumPy/reST |
| `load_agents_dir.py` 格式变更 | 只依赖已稳定的 frontmatter 字段；上游格式变更时走 schema 版本兼容 |
| 现有 CLI 兼容 | `pos convert` 输入是文件路径是否已存在目录判断，不会破坏现有字符串参数逻辑 |
| 总览 Agent 知识过时 | `clawcodex-overview.md` 由 `pos convert` 一次性生成，不会自动同步变更；需要重新运行 `pos convert` 刷新 |
| `--agent` 参数未指定且无 overview.md | 行为完全不变——`resolve_default_agent()` 返回 None，使用 GENERAL_PURPOSE_AGENT |
| 总览 Agent 自动加载影响已有行为 | `resolve_default_agent()` 返回 None 时不改变任何行为；`--agent general-purpose` 提供显式覆盖 |
| 自动检测增加启动延迟 | `resolve_default_agent()` 只做 `Path(clawcodex-overview.md).exists()` 检查，无额外 I/O 开销 |

##### 已拟定的设计决定

1. **`SourceCodeParser` 不对源码做语义分析**——只做结构化提取（类/方法/参数/docstring）。语义理解归 LLM。
2. **`SourceComponent` 是纯数据容器**——不包含业务逻辑，保持可序列化、可测试。
3. **`.claude/agents/*.md` 是本路径的默认输出格式**——`~/.clawcodex/agents/*.json` 保留为持久化格式，不做修改。
4. **`GroupStrategy` 支持组合**——例如 `component | io_relation` 先按模块分组，再跨模块关联同类型操作。
5. **保留 `_static_group()` 作为所有策略的 fallback**——当组件级分组匹配不到任何 `MappingRule` 时，不丢操作。
6. **技能参考脚本用文件嵌入而非模板渲染**——保持源码的原样性，避免 LLM 混淆。
7. **总览 Agent 命名使用 `clawcodex-overview`**——frontmatter `name: clawcodex-overview`，文件名 `clawcodex-overview.md`。直观标识为系统生成的总览入口，避免 `_` 前缀可能导致的文件发现兼容问题。已验证 `_list_markdown_files` 使用 `rglob("*.md")`，`_` / `-` 前缀均无问题，但 `clawcodex-` 命名更具可读性。
8. **`--agent` CLI 参数覆盖 > `clawcodex-overview.md` 自动检测 > 默认 `GENERAL_PURPOSE_AGENT`** ——三层优先级，满足不同使用场景。
9. **总览 Agent 的 system prompt 以 `append_system_prompt` 形式注入**——不替换整个 system prompt，保留 `build_full_system_prompt()` 的所有标准节（Available Agents、Available Tools、环境信息等）。这与 Coordinator 模式下替换整个 `engine_system_prompt` 不同，因为总览 Agent 需要保持基础设施上下文，而非替代它。
10. **总览 Agent 的 system prompt 是静态生成的**——不包含运行时动态指令（如当前阶段状态），避免 prompt 膨胀。运行时状态通过 CLAUDE.md 或用户消息传递。

##### 依赖与协同

- **依赖**：无。纯新增模块，不依赖其他 F-N。
- **协同**：
  - 与 `3.9.11 POS to Agent 转化模式`（概念层）互补：F-50 是代码实现层
  - 与 `3.9.12 业务 Agent 长期使用` 协同：`AgentMarkdownWriter` 的输出可通过 `persist_converted_agent()` 落盘
  - 与 `F-48（src/ 解耦）` 协同：`SourceCodeParser` 新增在 `extensions/pos_converter/` 下，不碰 `src/`。
  - 与 `load_agents_dir.py` 协同：确保生成的 `.md` 文件格式兼容 `load_agents_dir.py` 的 frontmatter 解析

---

### 3.19 Python SDK 方法注册为 Tool（F-52）
