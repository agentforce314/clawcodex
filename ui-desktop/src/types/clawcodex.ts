...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when ClawCodex doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in clawcodex_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `clawcodex model` uses — instead of
...
export interface ClawCodexConfig {
  agent?: {
...
export type ClawCodexConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `clawcodex model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) ClawCodex simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (clawcodex.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors clawcodex_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with ClawCodex, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `clawcodex tools` active-provider detection). */
  is_active: boolean
 ...
 *  not ClawCodex); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/clawcodex/update/check` — the backend's own update state.
