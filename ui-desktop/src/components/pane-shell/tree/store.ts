...
const STORAGE_KEY = 'clawcodex.desktop.layoutTree.v2'

writeKey('clawcodex.desktop.layoutTree.v1', null)
 ...
 */
export const $activePresetId = atom<string>(readKey('clawcodex.desktop.layoutPreset.active') ?? 'default')

export function markActivePreset(id: string) {
  $activePresetId.set(id)
  writeKey('clawcodex.desktop.layoutPreset.active', id)
}
 ...
 */
const DISMISSED_KEY = 'clawcodex.desktop.dismissedPanes.v1'
...
const USER_PLACED_KEY = 'clawcodex.desktop.userPlacedPanes.v1'
...
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__CLAWCODEX_LAYOUT_TREE__ = {
    close: closeTreePane,
