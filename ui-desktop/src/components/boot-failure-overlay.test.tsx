...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to ClawCodex gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.clawcodexDesktop
  Object.defineProperty(window, 'clawcodexDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'clawcodexDesktop', { configurable: true, value: original })
}
