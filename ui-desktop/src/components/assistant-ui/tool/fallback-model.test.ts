...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'ClawCodex Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'ClawCodex docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('ClawCodex Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'ClawCodex docs', url: 'https://example.com/docs' }
    ])
  })
})
  ...
  it('shows failed title when navigate returns success=false', () => {
    const view = buildToolView(
      part({
        ...
        args: { url: 'https://github.com/agentforce314/clawcodex/docs' },
    ...
    expect(view.title).toBe('Failed to open github.com/agentforce314/clawcodex/docs')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://github.com/agentforce314/clawcodex/docs' },
        result: { success: true, url: 'https://github.com/agentforce314/clawcodex/docs', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened github.com/agentforce314/clawcodex/docs')
  })
})
