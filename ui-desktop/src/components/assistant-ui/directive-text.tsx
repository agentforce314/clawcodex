...
const CLAWCODEX_REF_TYPES = WIRE_REFERENCE_KINDS
type ClawCodexRefType = (typeof CLAWCODEX_REF_TYPES)[number]
...
const CLAWCODEX_DIRECTIVE_RE = referenceRe()
...
export const clawcodexDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(CLAWCODEX_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...clawcodexDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing ClawCodex directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.clawcodexDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.clawcodexDesktop?.readFileDataUrl(id)
