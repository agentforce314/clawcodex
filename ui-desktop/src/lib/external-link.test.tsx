...
function installDesktopBridge(partial: Partial<Window['clawcodexDesktop']> = {}) {
...
function installTitleBridge(title: string) {
  ...
  installDesktopBridge({ fetchLinkTitle: bridge as unknown as Window['clawcodexDesktop']['fetchLinkTitle'] })
...
afterEach(() => {
  ...
  if (initialClawCodexDesktop) {
    ...
    delete desktopWindow.clawcodexDesktop
  }
})
...
describe('external link helpers', () => {
  ...
  it('opens links via the desktop bridge', () => {
    ...
    installDesktopBridge({ openExternal: openExternal as unknown as Window['clawcodexDesktop']['openExternal'] })
  ...
  it('prefixes a pretty link to a known host with its brand glyph', () => {
    ...
    const url = 'https://github.com/agentforce314/clawcodex/pull/123'
