import type {
  ClawCodexConnection,
  ClawCodexReadDirResult,
  ClawCodexReadFileTextResult,
  ClawCodexSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: ClawCodexConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: ClawCodexConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<ClawCodexReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<ClawCodexReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<ClawCodexReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<ClawCodexReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: ClawCodexSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
