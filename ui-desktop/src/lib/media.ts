...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.clawcodexDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.clawcodexDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.clawcodexDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.clawcodexDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaStreamUrl(path: string): string {
  return `clawcodex-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.clawcodex/cache, etc.); /api/media is intentionally
