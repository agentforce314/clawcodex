...
//
// When someone is in a hands-free "Hey ClawCodex" voice chat, the natural way to
...
// phrase (optionally addressed to ClawCodex), so a real turn that merely contains
...
// Optional address prefixes so "clawcodex stop" / "ok stop" / "hey clawcodex, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey clawcodex', 'hey clawcodex,', 'clawcodex', 'clawcodex,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("clawcodex") is not a stop command on its own.
      continue
    }
 ...
 * to ClawCodex). Returns false for anything that merely contains "stop" as part of
