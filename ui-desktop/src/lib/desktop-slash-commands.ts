...
/** Why a known ClawCodex command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active ClawCodex profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show ClawCodex version', surface: exec() },
...
function isKnownClawCodexSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * ClawCodex' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownClawCodexSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with ClawCodex and were never asked for. It is
