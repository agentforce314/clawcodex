...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'clawcodex:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.clawcodexDesktop?.saveImageFromUrl) {
        if (await window.clawcodexDesktop.saveImageFromUrl(src)) {
