import type { GatewayWsUrlResult } from '@clawcodex/shared'
...
declare global {
  interface Window {
    clawcodexDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<ClawCodexConnection>
      ...
      // ClawCodex Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new CLAWCODEX_CONFIG_DIR (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: ClawCodexApiRequest) => Promise<T>
      notify: (payload: ClawCodexNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<ClawCodexReadFileTextResult>
      selectPaths: (options?: ClawCodexSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<ClawCodexPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<ClawCodexPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<ClawCodexPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: ClawCodexActiveWork) => void
      setTitleBarTheme?: (payload: ClawCodexTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<ClawCodexReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<CLAWCODEX_CONFIG_DIR>/desktop-plugins),
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<ClawCodexGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<ClawCodexGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<ClawCodexGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<ClawCodexRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: ClawCodexReviewScope, baseRef?: null | string) => Promise<ClawCodexReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: ClawCodexReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<ClawCodexReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: ClawCodexTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<ClawCodexTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: ClawCodexWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: ClawCodexPreviewFileChanged) => void) => () => void
...
export interface ClawCodexTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface ClawCodexTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `clawcodex update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  clawcodexRoot?: string
...
export interface ClawCodexConnection {
  baseUrl: string
...
export interface ClawCodexTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface ClawCodexActiveWork {
  count: number
  titles: string[]
}

export interface ClawCodexWindowState {
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
  nativeOverlayWidth: number
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a ClawCodex Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted ClawCodex Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected ClawCodex Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the clawcodex gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered ClawCodex Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface ClawCodexApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface ClawCodexNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface ClawCodexPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface ClawCodexReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface ClawCodexPreviewWatch {
  id: string
  path: string
}
...
export interface ClawCodexGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface ClawCodexGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface ClawCodexGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface ClawCodexRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface ClawCodexRepoStatus {
  branch: null | string
...
// turn began.
export type ClawCodexReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface ClawCodexReviewFile {
  path: string
  added: number
  removed: number
...
export interface ClawCodexReviewList {
  files: ClawCodexReviewFile[]
...
export interface ClawCodexReviewPr {
  url: string
  state: string
  number: number
}
...
export interface ClawCodexReviewShipInfo {
  ghReady: boolean
  pr: ClawCodexReviewPr | null
}

export interface ClawCodexReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface ClawCodexReadDirResult {
  entries: ClawCodexReadDirEntry[]
  error?: string
}

export interface ClawCodexPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface ClawCodexSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
