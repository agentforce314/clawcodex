 ...
 *   (`@clawcodex/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default ClawCodexPlugin -> register(ctx)
 *
 ...
 * and `<clawcodex home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
import { type ClawCodexPlugin, createPluginContext } from './plugin'
...
/** Rewrite ONLY mapped import specifiers (@clawcodex/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @clawcodex/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: ClawCodexPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default ClawCodexPlugin export`)
    }
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<clawcodex home>/desktop-plugins/<name>/plugin.js`
...
async function scanDiskPlugins(): Promise<void> {
  ...
  try {
    ...
    // connected backend — a remote backend's clawcodex_home is a remote path and
...
export function watchRuntimePlugins(): void {
  ...
  const startDirWatch = async (): Promise<boolean> => {
    ...
    try {
      ...
      // clawcodex_home, which is a remote path in remote mode (#66899).
      const root = await desktop.desktopPluginsRoot?.()
