 ...
 * `ClawCodexPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/clawcodex'
...
export type { PluginRestOptions } from '@/clawcodex'
 ...
 *  under `clawcodex.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.clawcodexDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from ClawCodex — use `host.notify` for the in-app toast.
...
export interface ClawCodexPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `clawcodex.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.clawcodexDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.clawcodexDesktop
