...
import type { ClawCodexReviewFile, ClawCodexReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — ClawCodex' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'clawcodex.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'clawcodex.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'clawcodex.desktop.reviewTreeMode'
const SELECTED_KEY = 'clawcodex.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<ClawCodexReviewFile[]>([])
...
export const $reviewShipInfo = atom<ClawCodexReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['clawcodexDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: ClawCodexReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly ClawCodexReviewFile[], path: string): ClawCodexReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.clawcodexDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.clawcodexDesktop?.openExternal?.(url)
    }
