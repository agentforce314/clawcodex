...
export function setZoomPercent(percent: number): void {
  window.clawcodexDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.clawcodexDesktop?.zoom) {
  void window.clawcodexDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.clawcodexDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
