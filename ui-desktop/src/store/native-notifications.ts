...
const STORAGE_KEY = 'clawcodex:native-notifications'
...
// "Backgrounded" = the user isn't on ClawCodex. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.clawcodexDesktop?.notify({
 ...
 *  user is away from ClawCodex — the in-app toast (`host.notify`) covers the
 *  foreground case. */
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.clawcodexDesktop
