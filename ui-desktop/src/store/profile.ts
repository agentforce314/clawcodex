...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/clawcodex'
...
import type { ProfileInfo } from '@/types/clawcodex'
...
// /api/profiles/active `current`). "default" is the root ~/.clawcodex. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'clawcodex.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'clawcodex.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.clawcodexDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new CLAWCODEX_CONFIG_DIR. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.clawcodexDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.clawcodexDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'clawcodex.desktop.showAllProfiles'
...
// Switch to the default (root ~/.clawcodex) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.clawcodexDesktop?.touchBackend?.(target).catch(() => undefined)
}
