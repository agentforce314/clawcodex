import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@clawcodex/shared'
...
const isOpen = (gateway: ClawCodexGateway | null): boolean => gateway?.connectionState === 'open'
...
interface GatewayRegistryState {
  ...
  primaryGateway: ClawCodexGateway | null
  primaryProfile: string
  activeKey: string
  secondaries: Map<string, Secondary>
  $gateway: ReturnType<typeof atom<ClawCodexGateway | null>>
}

const STATE_KEY = Symbol.for('clawcodex.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<ClawCodexGateway | null>(null)
  }
}
...
export function setPrimaryGateway(gateway: ClawCodexGateway | null, profile = 'default'): void {
...
export function activeGateway(): ClawCodexGateway | null {
...
function createSecondary(profile: string): Secondary {
  const gateway = new ClawCodexGateway()
...
export async function ensureActiveGatewayOpen(): Promise<ClawCodexGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
