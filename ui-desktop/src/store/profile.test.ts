...
const { getProfiles } = await import('@/clawcodex')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/clawcodex/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<ClawCodexConnection> = {}): ClawCodexConnection =>
  ({ baseUrl: 'https://clawcodex-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as ClawCodexConnection

const localConn = (over: Partial<ClawCodexConnection> = {}): ClawCodexConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as ClawCodexConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<ClawCodexConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { clawcodexDesktop: { getConnection } })
