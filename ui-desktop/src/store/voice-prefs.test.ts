...
vi.mock('@/clawcodex', () => ({
  getClawCodexConfigRecord: vi.fn(async () => ({})),
  saveClawCodexConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye clawcodex', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye clawcodex')
  })
