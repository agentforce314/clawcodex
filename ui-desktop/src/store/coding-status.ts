...
import type { ClawCodexGitWorktree, ClawCodexRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: ClawCodexGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, ClawCodexRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, ClawCodexGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<ClawCodexRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<ClawCodexGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<ClawCodexRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<ClawCodexGitWorktree[]>>()
const $noRepoStatus = atom<ClawCodexRepoStatus | null>(null)
const $noWorktrees = atom<ClawCodexGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<ClawCodexRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<ClawCodexGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: ClawCodexRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: ClawCodexGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<ClawCodexRepoStatus | null>
  seq: number
  target: string
}
