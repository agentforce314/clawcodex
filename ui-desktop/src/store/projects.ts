...
import { type ClawCodexGateway, getClawCodexConfig } from '@/clawcodex'
import type { ClawCodexGitBaseBranch, ClawCodexGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/clawcodex'
...
const PROJECT_SCOPE_KEY = 'clawcodex.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('ClawCodex gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== ($activeGatewayProfile.get() || 'default')) {
    throw new Error('Active ClawCodex profile changed while connecting')
  }
...
async function refreshProjectTreeOn(gateway: ClawCodexGateway): Promise<void> {
...
const repoScanStates = new WeakMap<ClawCodexGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<ClawCodexGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getClawCodexConfig(context.profile))
...
// repo, returning where ClawCodex should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<ClawCodexGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<ClawCodexGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.clawcodexDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.clawcodexDesktop?.writeClipboard?.(path)
  }
}
