import type { ConnectionState } from '@clawcodex/shared'
import { atom, computed } from 'nanostores'

import { lastVisibleMessageIsUser } from '@/app/chat/thread-loading'
import type { ContextSuggestion } from '@/app/types'
import type { ClawCodexConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistBoolean, persistString, storedBoolean, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/clawcodex'
...
const COMPOSER_MODEL_KEY = 'clawcodex.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'clawcodex.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'clawcodex.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'clawcodex.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'clawcodex.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'clawcodex.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'clawcodex.desktop.lastRoute'
...
function workspaceCwdKey(connection: ClawCodexConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.clawcodexDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.clawcodexDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<ClawCodexConnection | null>(null)
...
export const setConnection = (next: Updater<ClawCodexConnection | null>) => updateAtom($connection, next)
...
// until config loads, and re-seeded on every profile switch by useClawCodexConfig.
