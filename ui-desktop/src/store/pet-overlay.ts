 ...
 * app's bounds and stays visible while ClawCodex is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'clawcodex.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'clawcodex.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.clawcodexDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.clawcodexDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.clawcodexDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.clawcodexDesktop?.petOverlay?.close()
}
