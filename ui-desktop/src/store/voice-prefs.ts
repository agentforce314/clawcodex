...
import { getClawCodexConfigRecord, saveClawCodexConfig } from '@/clawcodex'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getClawCodexConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveClawCodexConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
