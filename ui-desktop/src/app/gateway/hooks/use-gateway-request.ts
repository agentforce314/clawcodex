...
import type { ClawCodexGateway } from '@/clawcodex'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as ClawCodexGateway | null
  const gatewayRef = useRef<ClawCodexGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.clawcodexDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<ClawCodexGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as ClawCodexGateway | null
      }),
    []
  )
