...
// bug that drives the REAL useGatewayBoot hook + REAL ClawCodexGateway through a
...
function Harness({
  ...
  useGatewayBoot({
    ...
    refreshClawCodexConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { clawcodexDesktop?: unknown }).clawcodexDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { clawcodexDesktop?: unknown }).clawcodexDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForClawCodex) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startClawCodex()'s remote branch awaits waitForClawCodex() for 45s before it
    ...
    ;(window as { clawcodexDesktop?: unknown }).clawcodexDesktop = desktop
    ...
    // After ~45s waitForClawCodex gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('ClawCodex backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "ClawCodex couldn't start" overlay — the socket is open
