import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@clawcodex/shared'
import { useEffect, useRef } from 'react'

import { ClawCodexGateway } from '@/clawcodex'
...
import type { RpcEvent } from '@/types/clawcodex'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.clawcodexDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ClawCodexGateway | null) => void
  refreshClawCodexConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: ClawCodexConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting ClawCodex...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting ClawCodex…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to ClawCodex gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshClawCodexConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshClawCodexConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startClawCodex re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new ClawCodexGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshClawCodexConfig(),
          ...
          // retry it), not brick boot behind the "ClawCodex couldn't start"
