        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.clawcodexDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask ClawCodex to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.clawcodexDesktop?.watchPreviewFile ||
      !window.clawcodexDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.clawcodexDesktop.onPreviewFileChanged(payload => {
    ...
    void window.clawcodexDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.clawcodexDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.clawcodexDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:clawcodex-preview')
