...
import type { SessionInfo } from '@/clawcodex'
...
// zero ClawCodex sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, ClawCodexGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, ClawCodexGitWorktree[]>>({})
