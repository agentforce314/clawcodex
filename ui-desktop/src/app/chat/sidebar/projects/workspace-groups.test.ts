...
import type { ClawCodexGitWorktree } from '@/global'
import type { ProjectInfo, SessionInfo } from '@/types/clawcodex'
...
describe('baseName', () => {
  it('returns the final path segment, ignoring trailing slashes and separators', () => {
    expect(baseName('/www/clawcodex/')).toBe('clawcodex')
...
describe('mergeRepoWorktreeGroups (visual enhancer)', () => {
  it('injects a linked worktree lane discovered by git that has no sessions yet', () => {
    ...
    const discovered: ClawCodexGitWorktree[] = [
  ...
  it('surfaces a user-named "New worktree" under .worktrees/ as its own lane', () => {
    const discovered: ClawCodexGitWorktree[] = [
      {
        branch: 'clawcodex/test-gui-stuff',
        detached: false,
        isMain: false,
        locked: false,
    ...
    expect(merged.map(g => g.label)).toContain('clawcodex/test-gui-stuff')
  })

  it('relabels a dir-named linked worktree lane to its live checked-out branch', () => {
    // Backend labels the lane by the worktree dir (`clawcodex-ci`); the live
...
describe('overlayLiveLanes', () => {
  ...
  it('places a session into an out-of-tree (sibling) worktree lane by its path', () => {
    // `clawcodex-ci` is a linked worktree living BESIDE the repo, not under
