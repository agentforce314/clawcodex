...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { clawcodexDesktop: { readDir: typeof readDir } }).clawcodexDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): ClawCodexReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { clawcodexDesktop: unknown }).clawcodexDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: ClawCodexReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ClawCodexReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { clawcodexDesktop: unknown }).clawcodexDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.clawcodexDesktop is missing', async () => {
    delete (window as unknown as { clawcodexDesktop?: unknown }).clawcodexDesktop
