...
const CLAWCODEX_PATHS_MIME = 'application/x-clawcodex-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(CLAWCODEX_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(CLAWCODEX_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.clawcodexDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (clawcodex --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.clawcodexDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[clawcodex-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.clawcodexDesktop?.terminal?.write(sessionId, `${command}\r`)
