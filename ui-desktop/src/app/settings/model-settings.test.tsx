...
const getClawCodexConfigRecord = vi.fn()
const saveClawCodexConfig = vi.fn()
...
vi.mock('@/clawcodex', () => ({
  ...
  getClawCodexConfigRecord: () => getClawCodexConfigRecord(),
  saveClawCodexConfig: (config: unknown) => saveClawCodexConfig(config),
...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'nous', model: 'clawcodex-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'Nous',
        slug: 'nous',
        models: ['clawcodex-4', 'clawcodex-4-mini'],
        authenticated: true,
        capabilities: { 'clawcodex-4': { reasoning: true, fast: true } }
      }
    ]
  })
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'nous', model: 'clawcodex-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  getMoaModels.mockResolvedValue(null)
  setModelAssignment.mockResolvedValue({ provider: 'nous', model: 'clawcodex-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'nous', model: 'clawcodex-4', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getClawCodexConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveClawCodexConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  ...
  it('replaces the selected provider and model when the active profile changes', async () => {
    getGlobalModelInfo
      ...
      .mockResolvedValueOnce({ provider: 'nous', model: 'clawcodex-4' })
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getClawCodexConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveClawCodexConfig).toHaveBeenCalledWith(
      ...
      providers: [
        {
          ...
          models: ['clawcodex-4'],
          authenticated: true,
          capabilities: { 'clawcodex-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'nous', model: 'clawcodex-4' }]
    })
...
describe('ModelSettings MoA preset editor', () => {
  const moaConfig = () => ({
    ...
    presets: {
      default: {
        reference_models: [
          { provider: 'nous', model: 'clawcodex-4' },
  ...
  it('does not clear the model or save when the same provider is re-selected', async () => {
    ...
    try {
      ...
      expect(saveMoaModels).not.toHaveBeenCalled()
      expect(screen.getByText('nous · clawcodex-4')).toBeTruthy()
    } finally {
      vi.useRealTimers()
    }
  })
        ...
        expect.objectContaining({
          presets: expect.objectContaining({
            default: expect.objectContaining({
              reference_models: [
                expect.objectContaining({ provider: 'nous', model: 'clawcodex-4', enabled: false }),
