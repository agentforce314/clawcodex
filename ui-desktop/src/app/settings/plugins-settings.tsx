...
function reveal(file: string) {
  void window.clawcodexDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // clawcodex_home breaks against a remote backend (#66899).
    const dir = await window.clawcodexDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.clawcodexDesktop?.openDir?.(dir)
