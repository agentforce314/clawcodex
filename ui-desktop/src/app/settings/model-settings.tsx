...
import { invalidateClawCodexConfig, setClawCodexConfigCache, useClawCodexConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `clawcodex model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in clawcodex_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useClawCodexConfigRecord()
  ...
  const refresh = useCallback(async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
    ...
    try {
      ...
      void invalidateClawCodexConfig()
    } catch (err) {
      if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveClawCodexConfig(next)
      } catch (err) {
        setConfig(prev)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `clawcodex model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — ClawCodex runs the flow for you.`}
          </p>
        )}
