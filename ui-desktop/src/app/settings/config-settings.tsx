...
import { getClawCodexConfigSchema, getElevenLabsVoices, saveClawCodexConfig } from '@/clawcodex'
...
import type { ClawCodexConfigRecord, ConfigFieldSchema } from '@/types/clawcodex'

import { setClawCodexConfigCache, useClawCodexConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: ClawCodexConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<ClawCodexConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useClawCodexConfigRecord()
  ...
  const {
    ...
    queryKey: ['clawcodex-config-schema'],
    queryFn: getClawCodexConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveClawCodexConfig(config)
          ...
          setClawCodexConfigCache(config)
  ...
  const updateConfig = (next: ClawCodexConfigRecord) => {
