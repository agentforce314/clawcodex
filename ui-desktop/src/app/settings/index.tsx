...
import { getClawCodexConfigDefaults, getClawCodexConfigRecord, saveClawCodexConfig } from '@/clawcodex'
...
export function SettingsView({ onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getClawCodexConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'clawcodex-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await saveClawCodexConfig(await getClawCodexConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
