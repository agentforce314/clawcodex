...
import { disconnectOAuthProvider, listOAuthProviders } from '@/clawcodex'
import {
  FEATURED_ID,
  FeaturedProviderRow,
...
import type { EnvVarInfo, OAuthProvider } from '@/types/clawcodex'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.clawcodexDesktop?.terminal)
...
//      in clawcodex_cli/provider_catalog.py) — the SAME provider identity
//      `clawcodex model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // ClawCodex can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider ClawCodex can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // ClawCodex never deletes creds another tool owns behind a silent API call.
