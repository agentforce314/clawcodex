...
import type { ClawCodexConfigRecord, SessionInfo } from '@/types/clawcodex'
...
function AutoArchiveSetting() {
  ...
  useEffect(() => {
    ...
    if (!window.clawcodexDesktop) {
      return
    }

    let alive = true

    void getClawCodexConfigRecord()
      .then(record => {
        if (!alive) {
          return
        }
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await saveClawCodexConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files ClawCodex wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.clawcodexDesktop` is
    ...
    const settings = window.clawcodexDesktop?.settings
