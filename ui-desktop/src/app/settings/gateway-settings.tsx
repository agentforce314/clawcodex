...
// ClawCodex Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  // --- ClawCodex Cloud (cloud mode) state ---
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.clawcodexDesktop
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.clawcodexDesktop?.sshConfigHosts) {
      return
    }

    let cancelled = false
    void window.clawcodexDesktop
      .sshConfigHosts()
      .then(result => {
        if (!cancelled) {
  ...
  const payload = () => ({
    ...
    sshRemoteClawCodexPath: state.sshRemoteClawCodexPath.trim(),
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.clawcodexDesktop.applyConnectionConfig(payload())
        : await window.clawcodexDesktop.saveConnectionConfig(payload())
      ...
      const errors = {
        ...
        'clawcodex-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.clawcodexDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.clawcodexDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.clawcodexDesktop.getConnectionConfig(scope)
  ...
  const signOut = async () => {
    ...
    try {
      await window.clawcodexDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.clawcodexDesktop.getConnectionConfig(scope)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.clawcodexDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.clawcodexDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.clawcodexDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.clawcodexDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
  ...
  if (!window.clawcodexDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* ClawCodex Cloud panel: one portal sign-in, then a discovered-agent picker
                ...
                {cloudDiscover === 'loading' ? (
                  ...
                  <div className="flex items-start gap-2 py-3 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                    ...
                    <span>
                      ...
                      <ExternalLink href="https://portal.invalid/agents" showExternalIcon={false}>
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/clawcodex"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteClawCodexPath: event.target.value }))}
                placeholder={g.sshClawCodexPathPlaceholder}
                value={state.sshRemoteClawCodexPath}
              />
            }
            description={g.sshClawCodexPathDesc}
            title={g.sshClawCodexPathTitle}
          />
          {scope !== null ? (
            <ListRow
              action={
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          <ListRow
            action={
              <Button onClick={() => void window.clawcodexDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
