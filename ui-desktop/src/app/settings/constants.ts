...
export const EMPTY_SELECT_VALUE = '__clawcodex_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted ClawCodex & Nous-trained models',
    docsUrl: 'https://portal.invalid',
    priority: 0
  },
  {
    prefix: 'FIREWORKS_',
  ...
  { prefix: 'CLAWCODEX_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // clawcodex_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  agent: {
    ...
    maxTurns: 'Upper bound for tool-calling turns before ClawCodex stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters ClawCodex can read from one file request.',
  approvals: {
    mode: 'How ClawCodex handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When ClawCodex updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
