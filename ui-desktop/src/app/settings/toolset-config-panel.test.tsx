...
import type { ToolsetConfig } from '@/types/clawcodex'
...
const getClawCodexConfigSchema = vi.fn()
...
vi.mock('@/clawcodex', () => ({
  ...
  getClawCodexConfigSchema: () => getClawCodexConfigSchema(),
...
beforeEach(() => {
  ...
  getClawCodexConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getClawCodexConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveClawCodexConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveClawCodexConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
  ...
  describe('managed Nous provider activation', () => {
    ...
    it('drives the existing Nous OAuth device-code flow from the sign-in action and refetches', async () => {
      ...
      startOAuthLogin.mockResolvedValue({
        ...
        session_id: 'sess-1',
        user_code: 'NOUS-1234',
        verification_url: 'https://portal.invalid/device?user_code=NOUS-1234',
        poll_interval: 5,
        expires_in: 600
      })
      ...
      try {
        ...
        expect(openSpy).toHaveBeenCalledWith(
          'https://portal.invalid/device?user_code=NOUS-1234',
          '_blank',
          'noopener,noreferrer'
        )
