...
import { getSessionMessages, triggerCronJob } from '@/clawcodex'
...
import { useClawCodexConfig } from '../session/hooks/use-clawcodex-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshClawCodexConfig, sttEnabled, voiceMaxRecordingSeconds } = useClawCodexConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('clawcodex:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('clawcodex:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshClawCodexConfig(true)
    void refreshActiveProfile()
  }, [activeGatewayProfile, refreshCurrentModel, refreshClawCodexConfig])
