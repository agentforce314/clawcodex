...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.clawcodexDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.clawcodexDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.clawcodexDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.clawcodexDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // clawcodex:// deep links -> a reviewable /blueprint command in the composer.
  useEffect(() => {
    const unsubscribe = window.clawcodexDesktop?.onDeepLink?.(payload => {
    ...
    void window.clawcodexDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.clawcodexDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.clawcodexDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
