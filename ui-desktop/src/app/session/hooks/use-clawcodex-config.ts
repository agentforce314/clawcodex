...
import { getClawCodexConfig, getClawCodexConfigDefaults } from '@/clawcodex'
...
interface ClawCodexConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useClawCodexConfig({ activeSessionIdRef }: ClawCodexConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshClawCodexConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getClawCodexConfig(), getClawCodexConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying ClawCodex' built-in medium over the user's config.
  ...
  return { refreshClawCodexConfig, sttEnabled, voiceMaxRecordingSeconds }
}
