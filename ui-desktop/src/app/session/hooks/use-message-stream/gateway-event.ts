import type { BillingBlock } from '@clawcodex/shared'
import type { ClawCodexSkin } from '@clawcodex/shared/skin'
 ...
 * billing-specific toast — never the generic "ClawCodex error" — with a smart CTA
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshClawCodexConfig — two REST calls (config + defaults) per event, per
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        ingestBackendSkin((payload as { skin?: ClawCodexSkin } | undefined)?.skin, { apply: false })
        ...
        // A runtime skin switch (ClawCodex activating an authored skin, or `/skin`
        ...
        if (fromActiveProfile) {
          ingestBackendSkin(payload as ClawCodexSkin | undefined, { apply: true })
        }
        ...
        if (apply) {
          ...
          // everything refreshClawCodexConfig applies is either active-session
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'ClawCodex reported an error',
        ...
        // native OS notification when ClawCodex is backgrounded; dispatch is gated
        ...
        const errorMessage = payload?.message || 'ClawCodex reported an error'
