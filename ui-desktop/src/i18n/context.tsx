...
import { type ClawCodexConfigRecord, getClawCodexConfigRecord, saveClawCodexConfig } from '@/clawcodex'
...
export interface I18nConfigClient {
  getConfig: () => Promise<ClawCodexConfigRecord>
  saveConfig: (config: ClawCodexConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.clawcodexDesktop?.api) {
      return Promise.resolve({})
    }

    return getClawCodexConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.clawcodexDesktop?.api) {
    ...
    return saveClawCodexConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: ClawCodexConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: ClawCodexConfigRecord, locale: Locale): ClawCodexConfigRecord {
