/**
 * Live skin sync from the ClawCodex backend.
 *
 * The backend resolves the active skin (built-in or `$CLAWCODEX_CONFIG_DIR/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (ClawCodex
 ...
 */
export function ingestBackendSkin(skin: ClawCodexSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as ClawCodexSkin)
