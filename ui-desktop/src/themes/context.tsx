...
const SKIN_KEY = 'clawcodex-desktop-theme-v2'
const MODE_KEY = 'clawcodex-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'clawcodex-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'clawcodex-desktop-profile-modes-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.clawcodexTheme = skinName
  root.dataset.clawcodexMode = rendered
  ...
  window.clawcodexDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('clawcodex-boot-background', chromeBg)
    window.localStorage.setItem('clawcodex-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.clawcodexThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.clawcodexDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (ClawCodex authoring/activating a skin from a
