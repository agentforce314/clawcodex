...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.clawcodexHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.clawcodexHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.clawcodexHome)
  const builder = await RealSessionBuilder.start(sandbox.clawcodexHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.clawcodexHome, mock.url)
  fs.appendFileSync(path.join(sandbox.clawcodexHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.clawcodexHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
