 ...
 * AIAgent loop, using the E2E mock provider configured in `clawcodexHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(clawcodexHome: string) {
    this.child = spawn('uv', ['run', '--active', '--no-sync', 'python', '-m', 'tui_gateway.entry'], {
      cwd: REPO_ROOT,
      env: {
        ...process.env,
        CLAWCODEX_CONFIG_DIR: clawcodexHome,
  ...
  static async start(clawcodexHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(clawcodexHome)
