 ...
 * packaged binary) with a real `clawcodex serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → clawcodex serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with ClawCodex title', async () => {
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → clawcodex serve is spawned → WS connects → config
