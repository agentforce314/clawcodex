...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'ClawCodex E2E'], { cwd: repo })
...
function configureRepoCwd(clawcodexHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(clawcodexHome, mockUrl)
  fs.appendFileSync(path.join(clawcodexHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(clawcodexHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.clawcodexHome, mock.url, repo)
