/**
 * Shared E2E fixtures for the ClawCodex desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → clawcodex serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `clawcodex desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `clawcodex-e2e-${prefix}-${Math.random()}`))
  const clawcodexHome = path.join(root, 'clawcodex-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(clawcodexHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(clawcodexHome, 'config.yaml')
 ...
 */
export function writeEnvFile(clawcodexHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(clawcodexHome, '.env')
 ...
 */
function writeEmptyConfig(clawcodexHome: string): void {
  const configPath = path.join(clawcodexHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - CLAWCODEX_CONFIG_DIR → sandbox clawcodex-home (isolated config/sessions)
 *  - CLAWCODEX_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - CLAWCODEX_DESKTOP_IGNORE_EXISTING=1 → don't pick up `clawcodex` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - CLAWCODEX_DESKTOP_CLAWCODEX_ROOT → REPO_ROOT (dev checkout resolution)
 *  - CLAWCODEX_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    CLAWCODEX_CONFIG_DIR: sandbox.clawcodexHome,
    CLAWCODEX_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    CLAWCODEX_DESKTOP_IGNORE_EXISTING: '1',
    CLAWCODEX_DESKTOP_CLAWCODEX_ROOT: REPO_ROOT,
    CLAWCODEX_DESKTOP_APP_NAME: `ClawCodexE2E-${Date.now()}`,
    ...
    CLAWCODEX_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated CLAWCODEX_CONFIG_DIR + userData
 * @param env      - the process environment (already has CLAWCODEX_CONFIG_DIR etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(
    sandbox.clawcodexHome,
    mock.url,
    options.extraDisplayConfig,
    options.extraConfig,
    options.modelContextLength,
  )
  writeEnvFile(sandbox.clawcodexHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.clawcodexHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via CLAWCODEX_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`clawcodex serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.clawcodexHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { CLAWCODEX_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to ClawCodex backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'ClawCodex.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'ClawCodex.app', 'Contents', 'MacOS', 'ClawCodex')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'clawcodex')
}
 ...
 * progress without spawning a real ClawCodex backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * CLAWCODEX_CONFIG_DIR + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    CLAWCODEX_DESKTOP_BOOT_FAKE: '1',
    CLAWCODEX_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + clawcodex-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).CLAWCODEX_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).CLAWCODEX_DESKTOP_CLAWCODEX
  delete (env as Record<string, string | undefined>).CLAWCODEX_DESKTOP_CLAWCODEX_ROOT
