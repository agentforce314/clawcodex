...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `clawcodex-dev-mock-${Date.now()}`))
  ...
  return { root, clawcodexHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(clawcodexHome, mockUrl) {
  fs.writeFileSync(
    path.join(clawcodexHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(clawcodexHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.clawcodexHome, mock.url)
  console.log(`  CLAWCODEX_CONFIG_DIR: ${sandbox.clawcodexHome}`)
  ...
  const env = {
    ...
    CLAWCODEX_DESKTOP_APP_NAME: `ClawCodexDevMock-${Date.now()}`,
  }
