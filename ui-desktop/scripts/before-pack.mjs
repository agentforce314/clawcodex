 ...
 * product name (`ClawCodex`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/ClawCodex'
 *
 * This is a hard failure with no obvious cause for the user — `clawcodex desktop`
 ...
 * (the mac-unpacked ClawCodex.app bundle) and Windows (win-unpacked), so we
 ...
 * ClawCodex.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `clawcodex desktop --build-only` (clawcodex_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'ClawCodex.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'ClawCodex'}.exe`
