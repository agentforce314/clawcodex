#!/usr/bin/env node
// set-exe-identity.mjs — stamp the ClawCodex icon + version metadata onto the
// built ClawCodex.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked ClawCodex.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `clawcodex desktop`, the installer's
...
// path (which rebuilds via `clawcodex desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-ClawCodex.exe>
//
...
// Stamp the ClawCodex icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'ClawCodex',
      FileDescription: 'ClawCodex',
      CompanyName: 'ClawCodex contributors',
      LegalCopyright: 'Copyright (c) 2026 ClawCodex contributors'
    }
  })

  console.log('[set-exe-identity] done — ClawCodex icon + identity stamped')
}
