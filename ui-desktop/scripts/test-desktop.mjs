...
// no longer bundles the ClawCodex Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'ClawCodex.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'ClawCodex'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'ClawCodex.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'ClawCodex'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default CLAWCODEX_CONFIG_DIR for non-sandboxed runs -- matches main.ts's
// resolveClawCodexHome(). On Windows it's %LOCALAPPDATA%\clawcodex; elsewhere
// it's ~/.clawcodex. The fresh-install sandbox launchFresh() sets its own
// CLAWCODEX_CONFIG_DIR and never touches this.
const DEFAULT_CLAWCODEX_CONFIG_DIR = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'clawcodex')
  }
  return path.join(os.homedir(), '.clawcodex')
})()
const VENV_ROOT = path.join(DEFAULT_CLAWCODEX_CONFIG_DIR, 'clawcodex', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'clawcodex-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.CLAWCODEX_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `ClawCodex-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `ClawCodex-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `ClawCodex-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'ClawCodex-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.CLAWCODEX_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.CLAWCODEX_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const clawcodexHome = path.join(sandbox, 'clawcodex-home')
  ...
  env.CLAWCODEX_DESKTOP_CWD = cwd
  env.CLAWCODEX_DESKTOP_IGNORE_EXISTING = '1'
  env.CLAWCODEX_DESKTOP_TEST_MODE = 'fresh-install'
  env.CLAWCODEX_DESKTOP_USER_DATA_DIR = userDataDir
  env.CLAWCODEX_CONFIG_DIR = clawcodexHome
  delete env.CLAWCODEX_DESKTOP_CLAWCODEX
  delete env.CLAWCODEX_DESKTOP_CLAWCODEX_ROOT
  ...
  console.log(`  CLAWCODEX_CONFIG_DIR: ${clawcodexHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(clawcodexHome, 'clawcodex', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The ClawCodex Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of clawcodex_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'clawcodex', 'clawcodex_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing ClawCodex
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + CLAWCODEX_CONFIG_DIR
...
Fast rerun (skip rebuild if the packaged app already exists):
  CLAWCODEX_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
