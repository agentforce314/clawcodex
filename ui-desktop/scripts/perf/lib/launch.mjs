...
//   · its own CLAWCODEX_CONFIG_DIR      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · CLAWCODEX_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated CLAWCODEX_CONFIG_DIR with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate CLAWCODEX_CONFIG_DIR dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = clawcodexHome ?? mkTemp('clawcodex-perf-home-')
  const userData = userDataDir ?? mkTemp('clawcodex-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !clawcodexHome) {
    seedConfigFrom(join(homedir(), '.clawcodex'), home)
  }
  ...
  try {
    ...
    // CLAWCODEX_CONFIG_DIR (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set CLAWCODEX_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    const env = {
      ...process.env,
      CLAWCODEX_CONFIG_DIR: home,
      XCURSOR_SIZE: '24'
    }

    if (devUrl) {
      env.CLAWCODEX_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'clawcodex-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'clawcodex-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.clawcodex'), home)
