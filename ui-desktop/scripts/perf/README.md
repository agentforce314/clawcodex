...
# Its own --user-data-dir + CLAWCODEX_CONFIG_DIR means it never collides with `hgui`.
npm run perf -- --spawn
...
(separate lock scope), their own `CLAWCODEX_CONFIG_DIR` (separate backend + sessions),
