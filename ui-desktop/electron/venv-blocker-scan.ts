...
const SCAN_MODULE = 'clawcodex_cli._scan_venv_blockers'
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another ClawCodex process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the ClawCodex installation is free.\n' +
    '\n' +
    'Close other ClawCodex windows and terminals, then retry.  If the problem\n' +
    'persists, run `clawcodex update` in a terminal for detailed diagnostics.'
  )
}
