 ...
 * the three user-facing uninstall modes to the `clawcodex uninstall` CLI flags,
 ...
 *              `clawcodex uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `clawcodex uninstall --yes`
 *   - 'full' → remove everything: GUI + agent + all user data.
 *              `clawcodex uninstall --full --yes`
 *
 * Why a detached cleanup script: 'lite'/'full' delete the very venv the
 * `clawcodex` command runs from, and every mode may need to delete the running
...
/**
 * Map an uninstall mode to the `python -m clawcodex_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `clawcodex_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'clawcodex_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/ClawCodex.app/Contents/MacOS/ClawCodex  → …/ClawCodex.app
 *   Windows: …\ClawCodex\ClawCodex.exe                 → …\ClawCodex  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/ClawCodex.app/Contents/MacOS/ClawCodex → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/ClawCodex.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs ClawCodex.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]ClawCodex$/i.test(dir) || /[\\/]clawcodex-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/clawcodex
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import clawcodex_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, clawcodexHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export CLAWCODEX_CONFIG_DIR=${q(clawcodexHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import clawcodex_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but ClawCodex install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "CLAWCODEX_CONFIG_DIR=${String(clawcodexHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
