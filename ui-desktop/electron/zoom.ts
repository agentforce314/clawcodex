 ...
 * but forgot to emit 'clawcodex:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('clawcodex:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
