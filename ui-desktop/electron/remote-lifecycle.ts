 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote ClawCodex dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the ClawCodex install on the remote (login-shell probe),
 ...
 *     `CLAWCODEX_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForClawCodex in.
 *
 * The minted CLAWCODEX_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^CLAWCODEX_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.clawcodex/desktop-ssh'
...
// Resolve the remote clawcodex executable. An EXPLICIT path is honored strictly
...
async function locateClawCodex(ssh, remoteClawCodexPath) {
  ...
  if (remoteClawCodexPath) {
    if (await isExecutable(remoteClawCodexPath)) {
      return resolveLauncher(remoteClawCodexPath)
    }

    const err: any = new Error(
      `The ClawCodex path you set is not an executable on the remote host: "${remoteClawCodexPath}". ` +
        'Check the path (it must be the full path to the `clawcodex` binary on the remote, e.g. ' +
        '~/clawcodex/.venv/bin/clawcodex), or clear it to auto-detect.'
    )

    err.kind = 'clawcodex-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v clawcodex')}`)).trim()
  ...
  candidates.push('~/.local/bin/clawcodex')
  candidates.push('/usr/local/bin/clawcodex')
  candidates.push('~/.clawcodex/clawcodex/venv/bin/clawcodex')
  ...
  const err: any = new Error(
    'ClawCodex is not installed on the remote host (could not find a `clawcodex` executable). ' +
      'Install it on the remote with:  curl -fsSL https://github.com/agentforce314/clawcodex/install.sh | sh  ' +
      '— or set the ClawCodex path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<clawcodex> --version`,
// e.g. "ClawCodex v0.18.2 ..."), or '' on failure. Surfaces WHICH clawcodex a
// connection uses, so a stale/unexpected install is visible.
async function probeClawCodexVersion(ssh, clawcodexPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(clawcodexPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". ClawCodex Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The CLAWCODEX_CONFIG_DIR the remote dashboard will use (explicit env wins, else
// ~/.clawcodex). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteClawCodexHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${CLAWCODEX_CONFIG_DIR:-$HOME/.clawcodex}"')).trim().split('\n').pop()

    return out || '~/.clawcodex'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote ClawCodex home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'clawcodexPath', 'clawcodexHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(ssh, pid, spawnNonce, clawcodexPath = '') {
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !clawcodexPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(clawcodexPath)})\n` +
...
async function cleanupStale(ssh, ownershipId, lock, pidAlive = true) {
  if (pidAlive && lock && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.clawcodexPath))) {
    try {
      const result = (
        await ssh.exec(
...
function buildSpawnCommand(clawcodexPath, profile, opts: any = {}) {
  const clawcodex = expandRemotePath(clawcodexPath)
  ...
  const dashCmd = `env CLAWCODEX_DESKTOP=1 ${clawcodex} ${profileArgs}${subCmd}`
...
async function remoteSupportsSshOwnership(ssh, clawcodexPath) {
  const clawcodex = expandRemotePath(clawcodexPath)

  const out = await ssh.exec(
    `help="$(${clawcodex} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(ssh, { clawcodexPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, clawcodexPath))) {
    const err: any = new Error(
      'The remote ClawCodex install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update ClawCodex on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(clawcodexPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
 ...
 * opened SshConnection, forward/pickLocalPort/waitForClawCodex, a token-gated
...
async function connect(deps) {
  ...
  const clawcodexPath = await locateClawCodex(ssh, remoteClawCodexPath)
  log(`located clawcodex at ${clawcodexPath}`)
  const clawcodexVersion = await probeClawCodexVersion(ssh, clawcodexPath)

  if (clawcodexVersion) {
    log(`remote clawcodex version: ${clawcodexVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const clawcodexHome = await probeRemoteClawCodexHome(ssh)
  const lock = await readLockfile(ssh, ownershipId)

  if (lock) {
    const pidAlive = await remotePidAlive(ssh, lock.pid)
    const owned = pidAlive && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.clawcodexPath))
    ...
    const reusable =
      ...
      lock.clawcodexPath === clawcodexPath &&
      lock.clawcodexHome === clawcodexHome
  ...
  try {
    ...
    assertNotAborted(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForClawCodex(baseUrl, spawnToken)
    assertNotAborted(signal)
...
export {
  ...
  ownershipDirectory,
  pidIsOurDashboard,
  probeClawCodexVersion,
  probeRemoteClawCodexHome,
  probeRemotePlatform,
  PROTOCOL_VERSION,
  readLockfile,
  READY_RE,
