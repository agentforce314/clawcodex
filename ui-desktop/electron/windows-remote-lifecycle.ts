...
const READY_RE = /^CLAWCODEX_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitClawCodexPath = '') {
  const explicit = psLiteral(explicitClawCodexPath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$clawcodexHome=$env:CLAWCODEX_CONFIG_DIR',
    'if(-not $clawcodexHome){$clawcodexHome=Join-Path $env:LOCALAPPDATA "clawcodex"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command clawcodex.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $clawcodexHome "clawcodex\\venv\\Scripts\\clawcodex.exe")',
    '$candidates+=(Join-Path $HOME "clawcodex\\.venv\\Scripts\\clawcodex.exe")',
    '$clawcodex=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $clawcodex){throw "ClawCodex is not installed on the remote Windows host."}',
    'if($explicit -and $clawcodex -ne $explicit){throw "The configured ClawCodex path is not an executable file."}',
    '$python=Join-Path (Split-Path $clawcodex) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote ClawCodex Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;clawcodexHome=$clawcodexHome;clawcodexPath=$clawcodex;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitClawCodexPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitClawCodexPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'clawcodex_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.clawcodexPath === 'string' &&
    typeof lock.clawcodexHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.clawcodexPath === runtime.clawcodexPath &&
    lock.clawcodexHome === runtime.clawcodexHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertCurrent(signal)
  const runtime = await probeWindowsRemote(ssh, remoteClawCodexPath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.clawcodexPath])

  if (!inspection.supported) {
    const error: any = new Error('Update ClawCodex on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.clawcodexPath = inspection.path
  const clawcodexVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located clawcodex at ${runtime.clawcodexPath}`)
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, clawcodexPath: runtime.clawcodexPath })
    )
  } catch (error) {
  ...
  const owned = {
    ...
    profile,
    clawcodexPath: runtime.clawcodexPath,
    clawcodexHome: runtime.clawcodexHome,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForClawCodex(baseUrl, token)
    assertCurrent(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="ClawCodex SSH"', 'powershell.exe -NoLogo')
