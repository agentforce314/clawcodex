 ...
 *     `X-ClawCodex-Session-Token` header; WS uses `?token=`.
...
// clawcodex_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - clawcodex_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - clawcodex_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / clawcodex #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-clawcodex_session_at', '__Secure-clawcodex_session_at', 'clawcodex_session_at']
const RT_COOKIE_VARIANTS = ['__Host-clawcodex_session_rt', '__Secure-clawcodex_session_rt', 'clawcodex_session_rt']

// The Nous portal (NAS) does NOT use ClawCodex gateway session cookies — it is a
...
// Keep this aligned with clawcodex_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['clawcodex', 'test', 'tmp', 'root', 'sudo'])
...
// ClawCodex Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteClawCodexPath = String(entry.remoteClawCodexPath || '').trim()

  if (remoteClawCodexPath) {
    out.remoteClawCodexPath = remoteClawCodexPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote ClawCodex installation. Preserve an explicit mapping
  // when it is a valid ClawCodex profile identifier; otherwise fall back to the
 ...
 *     `--profile`, so its `CLAWCODEX_CONFIG_DIR` scopes it.
 *
...
function pathWithGlobalRemoteProfile(path, profile, opts: ProfileRouteOptions = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://clawcodex.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a clawcodex session ACCESS-token cookie
 ...
 * the ClawCodex gateway session cookies, so cloud sign-in / discovery liveness
