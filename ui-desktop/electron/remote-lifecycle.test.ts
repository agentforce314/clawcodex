...
function ownedLock(over: any = {}) {
  return {
    ...
    port: 40000,
    profile: '',
    clawcodexPath: '~/.local/bin/clawcodex',
    clawcodexHome: '~/.clawcodex',
...
test('locateClawCodex prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/clawcodex/, 'OK']])
  assert.equal(await locateClawCodex(ssh, '/opt/clawcodex'), '/opt/clawcodex')
})

test('locateClawCodex throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong clawcodex" bug.
  const ssh = fakeSsh([
    [/command -v clawcodex/, '/home/u/.local/bin/clawcodex\n'],
    [/\[ -x .*\.local\/bin\/clawcodex/, 'OK']
  ])

  await assert.rejects(
    () => locateClawCodex(ssh, '/bad/path/clawcodex'),
    (err: any) => {
      assert.equal(err.kind, 'clawcodex-not-found')
      assert.match(err.message, /\/bad\/path\/clawcodex/)
...
test('locateClawCodex falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v clawcodex/, '/home/u/.local/bin/clawcodex\n'],
    [/\[ -x .*\.local\/bin\/clawcodex/, 'OK']
  ])

  assert.equal(await locateClawCodex(ssh, ''), '/home/u/.local/bin/clawcodex')
})

test('locateClawCodex canonicalizes an installer wrapper to its executable target', async () => {
  const ssh = fakeSsh([
    [/command -v clawcodex/, '/home/u/.local/bin/clawcodex\n'],
    [/\[ -x .*\.local\/bin\/clawcodex/, 'OK'],
    [/python3 -c/, '/home/u/.clawcodex/clawcodex/venv/bin/clawcodex\n']
  ])

  assert.equal(await locateClawCodex(ssh, ''), '/home/u/.clawcodex/clawcodex/venv/bin/clawcodex')
})

test('locateClawCodex falls back to ~/.local/bin/clawcodex when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateClawCodex(ssh, ''), '~/.local/bin/clawcodex')
})

test('locateClawCodex tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/clawcodex/, 'OK']])
  assert.equal(await locateClawCodex(ssh, ''), '~/.clawcodex/clawcodex/venv/bin/clawcodex')
})

test('locateClawCodex throws a clawcodex-not-found error with an install hint', async () => {
...
test('locateClawCodex uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v clawcodex/, '/x/clawcodex'],
    [/\[ -x/, 'OK']
  ])

  await locateClawCodex(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.clawcodex/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.clawcodex/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.clawcodex/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/clawcodex'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/clawcodex serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/clawcodex'), true)
  assert.equal(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/clawcodex', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('CLAWCODEX_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { clawcodexPath: '/x/clawcodex', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('CLAWCODEX_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('CLAWCODEX_DASHBOARD_READY port=8765')?.[1], '8765')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { clawcodexPath: '/x/clawcodex', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nCLAWCODEX_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'CLAWCODEX_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'CLAWCODEX_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.clawcodex\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/print\("OWNED"/, 'OWNED\n'],
    [/kill 333/, ''],
    [/--version/, 'ClawCodex v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'CLAWCODEX_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile clawcodexPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ clawcodexPath: '/old/stale/clawcodex', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteClawCodexPath: '/new/clawcodex', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'CLAWCODEX_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes clawcodexHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/CLAWCODEX_CONFIG_DIR/, '/home/alice/.clawcodex\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'CLAWCODEX_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"clawcodexHome":"\/home\/alice\/\.clawcodex"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'CLAWCODEX_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'CLAWCODEX_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/clawcodex'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.clawcodex/clawcodex/venv/bin/clawcodex'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/clawcodex'))
  assert.doesNotThrow(() => validateRemotePath('~/.clawcodex/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/clawcodex'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('clawcodex'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/clawcodex'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/clawcodex\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/clawcodex\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/clawcodex\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/clawcodex', '/usr/`id`/clawcodex', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.clawcodex/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/clawcodex')
  assert.ok(result.includes('/usr/local/bin/clawcodex'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/clawcodex')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('CLAWCODEX_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/clawcodex', 'work', {
    tokenFilePath: `~/.clawcodex/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.clawcodex\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/clawcodex', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { clawcodexPath: '/x/clawcodex', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { clawcodexPath: '/x/clawcodex', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile rejects a log path outside the exact ownership and spawn path', async () => {
  const lock = ownedLock({ logPath: '~/.clawcodex/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.clawcodex/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/clawcodex serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/clawcodex serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/clawcodex'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/clawcodex'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/clawcodex'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/clawcodex'), false)
})
