...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(CLAWCODEX_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-ClawCodex-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.clawcodex/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'clawcodex-desktop-ssh')
  }

  return path.join(os.homedir(), '.clawcodex', 'desktop-ssh')
}
