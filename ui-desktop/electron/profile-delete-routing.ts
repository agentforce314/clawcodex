// Profile-delete routing logic for the `clawcodex:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_clawcodex_home() and
...
/**
 * Parse a `clawcodex:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `clawcodex:api` request away from the primary/window backend
 ...
 * ensure_clawcodex_home() recreates the directory the delete just removed.
 */
