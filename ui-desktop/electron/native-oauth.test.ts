...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/clawcodex', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/clawcodex/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/clawcodex'), 'https://gw.example.com/clawcodex/auth/native/refresh')
})
