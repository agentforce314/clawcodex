...
test('stagedUpdaterSupportsPrewrittenMarker rejects installers predating the self-adopt fix', () => {
  ...
  // refreshed because copy_self_to_clawcodex_home no-ops during --update.
  assert.equal(
    stagedUpdaterSupportsPrewrittenMarker('C:\\ClawCodex\\clawcodex-setup.exe', {
...
test('resolveStagedUpdaterBinary still returns a stale staged updater on Windows', () => {
  ...
  assert.equal(
    resolveStagedUpdaterBinary('C:\\ClawCodex', {
      fileExists: () => true,
      isWindows: true,
      stagedMtimeMs: () => MARKER_SELF_ADOPT_EPOCH_MS - 60 * DAY_MS
    }),
    path.join('C:\\ClawCodex', 'clawcodex-setup.exe')
  )
})
...
test('spawnUpdaterProcess hides the updater console and detaches the child on Windows', () => {
  ...
  const result = spawnUpdaterProcess(
    'clawcodex-setup.exe',
    ['--update', '--branch', 'main'],
    { cwd: 'C:\\ClawCodex', detached: true, stdio: 'ignore' },
    {
      isWindows: true,
  ...
  assert.deepEqual(calls, [
    {
      ...
      command: 'clawcodex-setup.exe',
      options: { cwd: 'C:\\ClawCodex', detached: true, stdio: 'ignore', windowsHide: true }
    }
  ])
})
...
test('resolveStagedUpdaterBinary hands Windows the staged installer it finds', () => {
  const home = 'C:\\Users\\clawcodex\\AppData\\Local\\clawcodex'
  const staged = path.join(home, 'clawcodex-setup.exe')
  const probed: string[] = []
...
test('resolveStagedUpdaterBinary returns null off Windows even when clawcodex-setup is staged (#74836)', () => {
  const home = '/Users/clawcodex/.clawcodex'
  let probes = 0

  const resolved = resolveStagedUpdaterBinary(home, {
    // The installer stages clawcodex-setup on macOS/Linux too, so "it exists" is
...
test('resolveStagedUpdaterBinary returns null on Windows when nothing is staged', () => {
  const resolved = resolveStagedUpdaterBinary('C:\\Users\\clawcodex\\AppData\\Local\\clawcodex', {
    fileExists: () => false,
    isWindows: true
  })
