 ...
 *   1. CLAWCODEX_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\clawcodex\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect CLAWCODEX_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.CLAWCODEX_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'clawcodex', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'clawcodex', 'git', 'usr', 'bin', 'bash.exe'))
  }
