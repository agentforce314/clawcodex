...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'clawcodex-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/clawcodex-runner-test',
    sourceRepoRoot: null,
    clawcodexHome: '/tmp/clawcodex-runner-test',
    logRoot: '/tmp/clawcodex-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'clawcodex', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'clawcodex')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/clawcodex',
      clawcodexHome: '/tmp/clawcodex'
    }),
    ['--dir', '/tmp/clawcodex', '--clawcodex-home', '/tmp/clawcodex', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/clawcodex', '--clawcodex-home', '/tmp/clawcodex', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/clawcodex',
      clawcodexHome: '/tmp/home'
    }),
    ['--dir', '/tmp/clawcodex', '--clawcodex-home', '/tmp/home', '--branch', 'main']
  )
})
