  ...
  const result = await detectRemotePlatform(
    sshWith(async command => {
      ...
      return JSON.stringify({
        ...
        arch: 'ARM64',
        clawcodexHome: 'C:\\h',
        clawcodexPath: 'C:\\h\\clawcodex.exe',
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('ClawCodex is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /ClawCodex is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\ClawCodex's\\python.exe" }, 'inspect', [
    'C:\\x y\\clawcodex.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'clawcodex_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /ClawCodex''s/)
  assert.match(script, /C:\\x y\\clawcodex\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { clawcodexPath: lock.clawcodexPath, clawcodexHome: lock.clawcodexHome }
