...
//   CLAWCODEX_WIN_SSH_HOST   ssh alias/host of the Windows box
//   CLAWCODEX_WIN_SSH_USER   remote user
//   CLAWCODEX_WIN_SSH_CLAWCODEX absolute path to the remote clawcodex.exe under test
const liveHost = process.env.CLAWCODEX_WIN_SSH_HOST || ''
const liveUser = process.env.CLAWCODEX_WIN_SSH_USER || ''
const configuredClawCodex = process.env.CLAWCODEX_WIN_SSH_CLAWCODEX || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-ClawCodex-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredClawCodex)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredClawCodex.replace('clawcodex.exe', 'python.exe')}' -m clawcodex_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredClawCodex.replace('clawcodex.exe', 'python.exe')
        const terminate = `& '${python}' -m clawcodex_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.clawcodexPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m clawcodex_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m clawcodex_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
