...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'clawcodex-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'clawcodex-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'ClawCodex Test'], { cwd: dir })
