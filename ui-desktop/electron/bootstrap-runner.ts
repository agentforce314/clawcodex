 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of ClawCodex by spawning
 ...
 *     logRoot,             // CLAWCODEX_CONFIG_DIR/logs
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.clawcodex-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(clawcodexHome) {
  return path.join(clawcodexHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.clawcodex/clawcodex. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(clawcodexHome) {
  if (!clawcodexHome) {
    return null
  }

  const candidate = path.join(clawcodexHome, 'clawcodex', 'scripts', installScriptName())
...
function cachedScriptPath(clawcodexHome, commit) {
  return path.join(bootstrapCacheDir(clawcodexHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/agentforce314/clawcodex/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(clawcodexHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(clawcodexHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, clawcodexHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass CLAWCODEX_CONFIG_DIR through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          CLAWCODEX_CONFIG_DIR: clawcodexHome || process.env.CLAWCODEX_CONFIG_DIR || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, clawcodexHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, clawcodexHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--clawcodex-home', clawcodexHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, clawcodexHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, clawcodexHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, clawcodexHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(clawcodexHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, clawcodexHome, emit })
