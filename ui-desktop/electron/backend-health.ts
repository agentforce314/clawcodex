...
export interface ClawCodexReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForClawCodexReady`).
   */
...
export async function waitForClawCodexReady(baseUrl: string, options: ClawCodexReadyOptions): Promise<void> {
  ...
  throw new Error(`ClawCodex backend did not become ready: ${detail}`)
}
