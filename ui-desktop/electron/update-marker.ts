...
export function markerPath(clawcodexHome) {
  return path.join(clawcodexHome, '.clawcodex-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(clawcodexHome)
  let raw
 ...
 *
 * The Tauri-based clawcodex-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(clawcodexHome, pid, { now = Date.now } = {}) {
  const file = markerPath(clawcodexHome)
 ...
 * ClawCodex to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(clawcodexHome, opts)
