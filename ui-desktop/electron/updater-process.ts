 ...
 *
 * The Tauri installer self-copies into CLAWCODEX_CONFIG_DIR on *every* platform
 * (`clawcodex-setup.exe` on Windows, `clawcodex-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_clawcodex_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `clawcodex update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(clawcodexHome, 'clawcodex-setup.exe')
 ...
 *
 * `copy_self_to_clawcodex_home` deliberately no-ops during `--update`
 ...
 * "Another ClawCodex update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `clawcodex update`, which
 ...
 * the installer is written to CLAWCODEX_CONFIG_DIR at install/repair time, making mtime
