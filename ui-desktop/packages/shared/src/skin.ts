/**
 * Canonical ClawCodex skin — the theme SDK's cross-surface contract.
 *
 * A skin is authored once as YAML in `$CLAWCODEX_CONFIG_DIR/skins/<name>.yaml` (or a
 * built-in), resolved by the Python skin engine (`clawcodex_cli/skin_engine.py`),
 ...
 *   • CLI     → `clawcodex_cli/skin_engine` → prompt_toolkit / Rich styles (Python)
 *
...
export interface ClawCodexSkin {
  name?: string
  description?: string
  colors?: SkinColors
