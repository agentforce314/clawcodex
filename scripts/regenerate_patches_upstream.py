...
* **new** — files that exist only under ``src/`` (fork-only additions). These
  ...
  (An earlier version of this script hard-coded ``new_files = []`` and silently
  dropped them — that was a bug under the strict-reconstruction invariant.)
...
* **(implicit preserve)** — any upstream file that is also in ``--preserve``
  ...
  version it is dropped. This covers both (a) files new in the upstream
  snapshot and (b) upstream-modified files the fork has chosen to keep as-is.
"""
...
def collect_preserve(args: argparse.Namespace) -> set[str]:
    preserve: set[str] = set()
    for rel in getattr(args, "preserve", []) or []:
        rel = rel.strip()
        if rel:
            preserve.add(rel)
    preserve_file = getattr(args, "preserve_file", None)
    if preserve_file and preserve_file.exists():
        for line in preserve_file.read_text().splitlines():
            line = line.strip()
...
def main() -> int:
    ...
    preserve = collect_preserve(args)

    # Cross-check: a preserved file MUST be in upstream; otherwise the user
    ...
    # Files present in upstream but absent from src/ are split into:
    #   - preserved: kept in base, no patch
    #   - deleted_by_fork: require a delete patch
    ...
    # Files present in BOTH src/ and upstream are split into:
    #   - preserved_existing: kept from base (the src/ version is dropped in
    #     favour of the upstream content; useful when the fork has no delta
    #     for an upstream-modified file)
    #   - modified: fork-side delta on top of upstream base
    ...
    modified_files = sorted(
        ...
        and files_differ_norm(upstream / relative_path, src / relative_path)
    )
    ...
    print(f"New files (fork-only): {len(new_files)}")
    print(f"Deleted files (by fork): {len(deleted_files) if args.allow_deletes else 0}")
    print(f"Preserved files (new in upstream base, kept): {len(preserved_files)}")
