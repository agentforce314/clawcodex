...
#### SR-1.1 会话与上下文管理（→ FEATURE_PLAN §4.3 结构化输出、§4.5 F-13 记忆隔离、§4.6 /goal、§4.10 sessionStorage、§4.11 F-12 cacheWarning）
...
#### SR-1.2 工具与技能执行（→ FEATURE_PLAN §4.4 MCP 扩展、§4.7 F-10 ExecuteExtraTool、§6.3 F-52 Python SDK Tool）
...
#### SR-1.4 权限与前端交互（→ FEATURE_PLAN §4.13 Auto 模式、§5.2 F-46 permission_mode、§5.3 F-47 Permission Schema）
...
### 2.2 IR-2 可观测、可调度与可维护底座（→ FEATURE_PLAN §4.1 进度汇报、§4.8 工具统计、§七 F-22 定时任务、§3.1.10 F-45、§3.1.12 F-51、§3.1.13 F-54、§6.1 F-48）

**抽象需求**: Agent 系统应支持任务进度上报、定时任务调度、长期运行稳定,并具备工具使用统计与策略优化的能力,确保无人值守场景下不失控、不沉默失败。

#### SR-2.1 任务进度与可观测（→ FEATURE_PLAN §4.1 Agent 进度汇报、§3.1.13 F-54 运行期可观测性）
...
#### SR-2.4 工具使用统计与策略（→ FEATURE_PLAN §4.8 工具/Skill 调用统计）
...
### 3.1 IR-3 研发自动化场景（→ FEATURE_PLAN §3.1.3~§3.2、§4.2、§4.12、§4.14）

把用户真实研发流程(Issue 处理、PR 评审、多 Agent 协作)自动化,并保证自动化失败时可被用户接管、纠偏、追溯。

#### SR-3.1 Issue → PR 编排（→ FEATURE_PLAN §3.1.3 F-36 LocalTracker、§3.1.9 F-42 Workspace 策略、§3.1.14 F-44 人工检视闸门、§3.2 Orchestrator CLI）
...
#### SR-3.2 澄清、重跑与人机协同（→ FEATURE_PLAN §3.1.6 F-39 Issue 重跑、§3.1.11 F-49 会话统一存储、§4.12 Issue 语义澄清流程）
...
#### SR-3.4 多 Agent 编排与 A2A 协作（→ FEATURE_PLAN §3.1.8 F-41 Coordinator、§4.2 Team 管理、§4.14 Agent 间交互）
...
### 3.2 IR-4 业务 Agent 与远程值守（→ FEATURE_PLAN §4.9、§6.2、§6.3）

把专业系统(POS)和远程值守转成可长期运行、可被 ClawCodex 调度的业务 Agent,支持主 Agent 切换、daemon 模式与跨设备 attach。

#### SR-4.1 POS to Agent（→ FEATURE_PLAN §4.9 CreateAgentTool、§6.2 F-50 POS 转换器固化、§6.3 F-52 Python SDK Tool）
