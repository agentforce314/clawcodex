...
### 2.1 IR-1 Agent 可运行底座（→ FEATURE_PLAN §3.4~§3.12 各节）

**抽象需求**: Agent 应当作为可独立运行的最小软件单元存在,具备完整的会话生命周期、工具发现与执行、模型灵活切换、终端与远程交互、后台与恢复能力。

#### SR-1.1 会话与上下文管理（→ FEATURE_PLAN §3.4 结构化输出、§3.6 F-13 记忆隔离、§3.7 /goal、§3.11 sessionStorage、§3.12 F-12 cacheWarning）
...
#### SR-1.2 工具与技能执行（→ FEATURE_PLAN §3.5 MCP 扩展、§3.8 F-10 ExecuteExtraTool、§3.22 F-52 Python SDK Tool）
...
#### SR-1.3 模型与 Provider 接入（→ FEATURE_PLAN §3.17 F-43 CLI 模型切换）
...
#### SR-1.4 权限与前端交互（→ FEATURE_PLAN §3.14 Auto 模式、§3.18 F-46 permission_mode、§3.19 F-47 Permission Schema）
...
#### SR-1.5 后台、恢复与远程桥接（→ FEATURE_PLAN §五 F-55 会话恢复增强）
...
### 2.2 IR-2 可观测、可调度与可维护底座（→ FEATURE_PLAN §3.2 进度汇报、§3.9 工具统计、§四 F-22 定时任务、§3.1.10 F-45、§3.1.12 F-51、§3.1.13 F-54、§3.20 F-48）

**抽象需求**: Agent 系统应支持任务进度上报、定时任务调度、长期运行稳定,并具备工具使用统计与策略优化的能力,确保无人值守场景下不失控、不沉默失败。

#### SR-2.1 任务进度与可观测（→ FEATURE_PLAN §3.2 Agent 进度汇报、§3.1.13 F-54 运行期可观测性）
...
#### SR-2.2 定时任务与调度（→ FEATURE_PLAN §四 F-22 定时任务系统）
...
#### SR-2.3 稳定性与开放替代（→ FEATURE_PLAN §3.1.10 F-45 Tool-call 审计、§3.1.12 F-51 空转检测、§3.20 F-48 src/ 解耦）
...
#### SR-2.4 工具使用统计与策略（→ FEATURE_PLAN §3.9 工具/Skill 调用统计）
...
### 3.1 IR-3 研发自动化场景（→ FEATURE_PLAN §3.1.3~§3.1.16、§3.3、§3.13、§3.15）

把用户真实研发流程(Issue 处理、PR 评审、多 Agent 协作)自动化,并保证自动化失败时可被用户接管、纠偏、追溯。

#### SR-3.1 Issue → PR 编排（→ FEATURE_PLAN §3.1.3 F-36 LocalTracker、§3.1.9 F-42 Workspace 策略、§3.1.14 F-44 人工检视闸门、§3.16 Orchestrator CLI）
...
#### SR-3.2 澄清、重跑与人机协同（→ FEATURE_PLAN §3.1.6 F-39 Issue 重跑、§3.1.11 F-49 会话统一存储、§3.13 语义澄清）
...
#### SR-3.4 多 Agent 编排与 A2A 协作（→ FEATURE_PLAN §3.1.8 F-41 Coordinator、§3.3 Team 管理、§3.15 Agent 间交互）
...
### 3.2 IR-4 业务 Agent 与远程值守（→ FEATURE_PLAN §3.10、§3.21、§3.22）

把专业系统(POS)和远程值守转成可长期运行、可被 ClawCodex 调度的业务 Agent,支持主 Agent 切换、daemon 模式与跨设备 attach。

#### SR-4.1 POS to Agent（→ FEATURE_PLAN §3.10 CreateAgentTool、§3.21 F-50 POS 转换器固化、§3.22 F-52 Python SDK Tool）
...
### 4.1 IR-5 自升级闭环（→ FEATURE_PLAN §六 CCB 对标）
