...
export default class Ink {
   ...
   */
  /** Test-only: stringify the current front frame as plain text (no SGR), for the
   *  ink-testing-library shim. Reuses charInCellAt — the same screen→text primitive
   *  the clipboard/selection path uses. Trailing blanks/rows trimmed. */
  lastFrameText(): string {
    const s = this.frontFrame.screen
    const lines: string[] = []
    for (let y = 0; y < s.height; y++) {
      let row = ''
      for (let x = 0; x < s.width; x++) row += charInCellAt(s, x, y) ?? ' '
